 ...
 */
DejaClickUi.ElementPath = function (aEvent, aIsFullXPath, aRootElement, aWindow, aUtils,
   ...
   for (index = 0; index < targetElts.length; ++index) {
      if (targetElts[index].getAttribute('type') === 'element') {
         path = aScript.getChildWithTag(targetElts[index], aIsFullXPath?'elementfullxpath':'elementpath');
         if (path !== null) {
   ...
   function initialize() {
      try {
         DejaClickUi.elementPath = new DejaClickUi.ElementPath(
            ...
            DejaClick.service.__modal.arguments.isFullXPath,
         ...
         DejaClick.service.__modal.setTitle('deja_elementPath_title');
      } catch (ex) {
         DejaClick.utils.logger.logException(ex);
      }
   }
