...
DejaClickUi.Properties = function (aDejaClick, aUtils, aConstants, aGetScript, aOpenDialog,
   ...
   this.properties = {
      ...
      triggers: new DejaClickUi.KeywordValidationsProperty(this.context, true),
      triggerOptions: new DejaClickUi.TriggerOptionsProperty(this.context),
      //notes: new DejaClickUi.NotesProperty(this.context),
      kwValidations: new DejaClickUi.KeywordValidationsProperty(this.context, false),
      ...
      cookies: new DejaClickUi.CookiesProperty(this.context),
      ...
      jsDialogOptions: new DejaClickUi.JSDialogOptionsProperty(this.context),
   ...
   //UXM-11101 - Make this element visible after everything has been initialized
   this.elements.accordion.css("visibility", "visible");
};
...
DejaClickUi.PropertyContext.prototype = {
    ...
    *    current context.
    */
   updatePropertyModified: function (aProperty , aPropertyElement, aPreferences) {
   ...
   syncValidationsChange: function (aProperty) {
    ...
    /**
    * Trigger the dejaclick:propertyupdated event for the trigger
    * property
    * @this {DejaClickUi.PropertyContext}
    */
   syncTriggersChange: function (aProperty) {
      this.syncPropertyChange('triggers');
   },
...
DejaClickUi.EventInputProperty = function (aContext) {
   ...
   this.elements = {
      ...
      clickEventsDiv: $('#eventInputClickEventsDiv'),
      clickEventsInput: $('#eventInputClickEvents'),
      ...
      simulateEnterDiv: $('#eventInputSimulateEnterDiv'),
      simulateEnter: $('#eventInputSimulateEnter'),
   ...
   this.elements.clickEventsInput.on('input', this.changeValue.bind(this));
   ...
   this.elements.simulateEnter.on('change', this.changeValue.bind(this));
...
DejaClickUi.EventInputProperty.prototype = {
   ...
   refresh: function () {
      ...
      case 'script':
         ...
         break;
      case 'event':
         var eventType = this.context.event.getAttribute('type');
         switch (eventType) {
            case 'navigate':
            ...
            case 'click':
            case 'mousedown':
               if ( this.context.displayLevel === this.context.constants.DISPLAYLEVEL_DIAGNOSTIC ) {
                  this.refreshClickEvent();
               } else {
            ...
            case 'change':
               ...
               if ( eventType == 'keyboard' ) {
                  ...
                  var isShadow = this.context.getEventParam('shadowPath')!== null;
                  this.state.inputTag = isShadow ? 'input':this.state.inputTag;
   ...
   refreshGlobal: function () {
      ...
      this.elements.simulateEnter.prop('checked',
         this.context.getPreference('DC_OPTID_SIMULATEENTER'));
      ...
      this.elements.simulateEnterDiv.show();
      this.elements.optionDiv.show();

      this.elements.urlDiv.hide();
      this.elements.clickEventsDiv.hide();
      ...
      this.state.hasValue = this.context.updatePropertyModified(this,
   ...
   /**
    * Update the UI for a click event.
    * @this {DejaClickUi.EventInputProperty}
    */
   refreshClickEvent: function () {
      var value, isDefaultValue;

      value = this.context.getEventParam('sequence');
      if (value == null) {
         var eventType = this.context.event.getAttribute('type');
         if ( eventType == 'mousedown' ) {
            //Default value for mousedown sequence
            value = 'mousedown';
         } else {
            //Default value for click sequence
            value = 'mousedown,focus,mouseup,click';
         }
      }

      // Display URL or variable value.
      this.state.textValue = '';
      this.state.textField = this.elements.clickEventsInput;
      this.state.textField.attr('rows', (value.length > 30) ? '3' : '1');
      this.state.textField.removeAttr('readonly');
      this.state.textField.val(value);

      // Show/hide the appropriate UI elements.
      this.elements.clickEventsDiv.show();
   ...
   refreshTextChangeEvent: function () {
      ...
      if (value == null) {
         ...
         var shadowValue = this.context.getEventParam('shadowValue');
         value = shadowValue !== null ? shadowValue : value;
         if (value == null) {
            value = '';
         }
      } else {
      ...
      if (this.state.inputType === 'password') {
         ...
         if (this.context.event.getAttribute('screen') === 'true') {
            ...
            this.elements.simulateEnterDiv.hide();
   ...
   refreshContentEditableChangeEvent: function () {
      var value, isScriptVar, useKeystrokes, simulateEnter;
      ...
      simulateEnter = this.context.getPreference('DC_OPTID_SIMULATEENTER');
      this.elements.simulateEnter.prop('checked', simulateEnter);
      ...
      // Enable the correct controls.
      this.state.changed = this.context.updatePropertyModified(this,
         this.elements.section, [ 'DC_OPTID_USEKEYSTROKES' ]);
      this.enableControls();
   },
   ...
   apply: function (opt_event) {
      ...
      try {
         ...
         case 'event':
            ...
            case 'click':
            case 'mousedown':
               applied = this.applyClickSequenceEvent();
               break;
            case 'keyboard':
   ...
   /**
    * Commit the changes to a click/mousedown event.
    * @this {DejaClickUi.EventInputProperty}
    * @return {boolean} true if the change was applied.
    */
   applyClickSequenceEvent: function () {
      var sequence;

      sequence = this.elements.clickEventsInput.val().trim().replace(/\n/g, '');
      ...
      if ( seqArray && seqArray.length > 1 ) {
         for (var i=0; i < seqArray.length; i++) {
            var seqEvent = seqArray[i].trim();
            if ( seqEvent == "focus" || seqEvent == "mousedown" || seqEvent == "mouseover" 
                  || seqEvent == "mouseup" || seqEvent == "click" ) 
            {
               if ( finalSequence != "" ) {
                  finalSequence += ",";
               }
               finalSequence += seqEvent;
            } else {
               validSequence = false;
               break;
            }
      ...
      if ( ! validSequence ) {
         window.alert(this.context.getMessage(
               'deja_properties_enterValidClickEventsSequence'));
         return false;
      } 

      
      this.context.setEventParam('sequence', finalSequence);

      this.state.textField = this.elements.clickEventsInput;
      this.state.textField.attr('rows', (finalSequence.length > 30) ? '3' : '1');
      this.state.textField.val(finalSequence);
   ...
   applyTextChangeEvent: function (aParam) {
      ...
      if (this.elements.useScriptVar.prop('checked')) {
         ...
         aParam = this.context.getEventParam('shadowValue') === null ? aParam : "shadowValue";
      ...
      this.context.setPreference('DC_OPTID_USEKEYSTROKES',
         this.elements.useKeystrokes.prop('checked'));
      this.context.setPreference('DC_OPTID_SIMULATEENTER',
         this.elements.simulateEnter.prop('checked'));
      return true;
   },
 ...
 * 
 * UXM-11948 - Updated to support the edit of subscript triggers too.
 * 
 * @constructor
 * @implements {DejaClickUi.Property}
 * @param {!DejaClickUi.PropertyContext} aContext The context to which
 *    the property applies.
 */
DejaClickUi.KeywordValidationsProperty = function (aContext, triggerEditScreen) {
   this.context = aContext;

   this.isTrigger = triggerEditScreen?true:false;

   this.validations = [];

   this.elements = {
      description: this.isTrigger?$('#triggerDescription'):$('#keywordValidationDescription'),
      select: this.isTrigger?$('#triggerList'):$('#keywordValidationList'),
      add: this.isTrigger?$('#triggerAdd'):$('#keywordValidationAdd'),
      edit: this.isTrigger?$('#triggerEdit'):$('#keywordValidationEdit'),
      remove: this.isTrigger?$('#triggerRemove'):$('#keywordValidationRemove'),
      allButtons: this.isTrigger?$('#triggerSection').find('button'):$('#keywordValidationSection').find('button')
   };
...
DejaClickUi.KeywordValidationsProperty.prototype = {
   ...
   refresh: function () {
      ...
      if ( this.isTrigger ) {
         this.elements.description.text(this.context.getMessage('deja_properties_triggerLabel'));
      } else {
      ...
      if (validationsElt == null) {
         ...
         this.validations = Array.prototype.filter.call(
            this.isTrigger?validationsElt.getElementsByTagName('trigger'):validationsElt.getElementsByTagName('validation'),
   ...
   completeEdit: function (aValidation) {
      try {
         if (aValidation !== null) {
            if ( this.isTrigger ) {
               this.context.syncTriggersChange();
            } else {
         ...
         if (selected.length !== 0) {
            ...
            if ((0 <= index) && (index < this.validations.length)) {
               ...
               if (validationsElt.firstElementChild == null) {
                  ...
                  script.renumberElements(this.isTrigger?'trigger':'validation');
               }
            }
            if ( this.isTrigger ) {
               this.context.syncTriggersChange();
...
/**
 * Subscript trigger options. To define the maximum number of times
 * a subscript could be triggered and it it should fail after the maximum
 * times is reached
 * 
 * @constructor
 * @implements {DejaClickUi.Property}
 ...
 *    the property applies.
 */
DejaClickUi.TriggerOptionsProperty = function (aContext) {
   this.context = aContext;
   this.prefService = aContext.prefService;

   this.maxReplaysAttrName = this.prefService.getName('DC_OPTID_TRIGGER_MAX_REPLAYS');
   this.skipReplaysAttrName = this.prefService.getName('DC_OPTID_TRIGGER_SKIP_AFTER_MAX_REPLAYS');
   ...
   this.elements = {
      section: $('#triggerOptions'),
      maxReplays: $('#triggerMaxReplays'),
      skipTriggerOnMaxReplays: $('#skipTriggerOnMaxReplays'),
      failTriggerOnMaxReplays: $('#failTriggerOnMaxReplays'),
      reset: $('#triggerOptionsReset'),
      apply: $('#triggerOptionsApply')
   };
   this.elements.allInputs = this.elements.section.find('input');
   this.elements.allButtons = this.elements.section.find('button');
...
DejaClickUi.TriggerOptionsProperty.prototype = {
   /** Constructor for objects with this prototype. */
   constructor: DejaClickUi.EventTimeoutProperty,
   ...
   refresh: function () {
      ...
      var triggers = this.context.script.getChildWithTag(this.context.element, 'triggers');
      if ( triggers && triggers.hasAttribute(this.maxReplaysAttrName) ) {
         this.elements.maxReplays.val(String(triggers.getAttribute(this.maxReplaysAttrName)));
      } else {
         this.elements.maxReplays.val(String(this.prefService.getDefault('DC_OPTID_TRIGGER_MAX_REPLAYS')));
      }

      var skipReplay = true;
      if ( triggers && triggers.hasAttribute(this.skipReplaysAttrName) ) {
         skipReplay = triggers.getAttribute(this.skipReplaysAttrName) == 'true';
      } else {
         skipReplay = this.prefService.getDefault('DC_OPTID_TRIGGER_SKIP_AFTER_MAX_REPLAYS');
      }
      
      if (skipReplay) {
         this.elements.skipTriggerOnMaxReplays.prop('checked', true);
      } else {
         this.elements.failTriggerOnMaxReplays.prop('checked', true);
      }
   ...
   changeValue: function (aEvent) {
      try {
         if (!this.state.hasNewValue ||
               (aEvent.target.id === 'triggerMaxReplays')) {
            this.state.hasNewValue = true;
            this.enableControls();
         }
      } catch (ex) {
   ...
   reset: function (opt_event) {
      try {
         ...
         if (this.prefService.getDefault('DC_OPTID_TRIGGER_SKIP_AFTER_MAX_REPLAYS')) {
            this.elements.skipTriggerOnMaxReplays.prop('checked', true);
         } else {
            this.elements.failTriggerOnMaxReplays.prop('checked', true);
         }
   ...
   apply: function (opt_event) {
      ...
      try {
         var triggers = this.context.script.getChildWithTag(this.context.element, 'triggers');
         if ( ! triggers ) {
            triggers = this.context.script.domTreeInsertNode(this.context.element, 'triggers', null);
         }

         maxReplays = Number(this.elements.maxReplays.val());
         if (!this.context.validate('DC_OPTID_TRIGGER_MAX_REPLAYS', maxReplays)) {
            window.alert(this.context.getMessage(
               'deja_properties_invalidEntry'));
            return;
         }

         triggers.setAttribute(this.maxReplaysAttrName, maxReplays);
         triggers.setAttribute(this.skipReplaysAttrName, this.elements.skipTriggerOnMaxReplays.prop('checked'));

         this.context.syncPropertyChange('triggerOptions');
...
DejaClickUi.DialogPromptsProperty.prototype = {
   ...
   refresh: function () {
      ...
      if ( typeof browser !== "undefined" ) {
         this.elements.description.text(this.context.getMessage('deja_properties_notSupportedAtFirefox'));
         this.elements.select.attr('disabled', 'true');
         this.elements.allButtons.button('option', 'disabled', true);
   ...
   enableControls: function () {
      var noSelection;
      if (!this.context.controlsEnabled 
         || typeof browser !== "undefined" ) //TODO Firefox Quantum UXM-11026 - Dialog prompts not supported for Firefox Quantum
      {
         this.elements.select.attr('disabled', 'true');
         this.elements.allButtons.button('option', 'disabled', true);
      } else {
...
DejaClickUi.MatchOptionsProperty = function (aContext) {
   ...
   this.elements = {
      ...
      elementFullXPath: $('#matchOptionsFullXPath'),
      editFullXPath: $('#matchOptionsEditFullXPath'),
   ...
   this.elements.elementFullXPath.on('change', this.changeMatchType.bind(this));
   ...
   this.elements.editFullXPath.on('click', this.editFullXPath.bind(this));
...
DejaClickUi.MatchOptionsProperty.prototype = {
   ...
   close: function () {
      if (this.hasOwnProperty('elements')) {
         ...
         this.elements.editFullXPath.off('click');
         this.elements.allInputs.off('change input');
      }
      delete this.elements;
      delete this.state;
    ...
    */
   refresh: function () {
      var useMatchTypes, fingerprint, elementPath, elementFullXPath, breadcrumbs, diagnostic;
      ...
      if (useMatchTypes.indexOf('all') !== -1) {
         fingerprint = elementPath = elementFullXPath = breadcrumbs = true;
      } else {
         fingerprint = useMatchTypes.indexOf('fp') !== -1;
         elementPath = useMatchTypes.indexOf('ep') !== -1;
         elementFullXPath = useMatchTypes.indexOf('fx') !== -1;
      ...
      this.elements.elementFullXPath.prop('checked', elementFullXPath);
      ...
      if (this.context.category === 'event') {
         ...
         if (diagnostic) {
            ...
            // Show if event has a target with an elementfullxpath.
            this.elements.editFullXPath.toggle($(this.context.event).
               has('target[type="element"] elementfullxpath').length !== 0);
         } else {
            this.elements.editElementPath.hide();
            this.elements.editFullXPath.hide();
         }
      } else {
   ...
   selectMatchType: function (aEvent) {
      try {
         ...
         case 'optimized':
            ...
            this.elements.elementFullXPath.prop('checked', true);
         ...
         case 'intensive':
            ...
            this.elements.elementFullXPath.prop('checked', false);
            this.elements.breadcrumbs.prop('checked', true);
            break;
         }
         this.changeValue(aEvent);
   ...
   editElementPath: function (aEvent) {
      try {
         this.context.openDialog('ui/content/dejaElementPath.html',
            { context: this.context.element, isFullXPath: false },
            this.completeEdit.bind(this));
      } catch (ex) {
         this.context.logger.logException(ex);
      }
   ...
   /**
    * Open a dialog to edit the element Full XPath to the target for this
    * current event.
    * @this {DejaClickUi.MatchOptionsProperty}
    * @param {!Event} aEvent A jQuery click event on the edit elementfullxpath link.
    */
   editFullXPath: function (aEvent) {
      try {
         this.context.openDialog('ui/content/dejaElementPath.html',
            { context: this.context.element, isFullXPath: true },
            this.completeEdit.bind(this));
      } catch (ex) {
         this.context.logger.logException(ex);
      }
    ...
    */
   apply: function (opt_event) {
      var minScore1, minScore2, fingerprint, elementPath, elementFullXPath, breadcrumbs,
         matchTypes;
      ...
      try {
         ...
         elementFullXPath = this.elements.elementFullXPath.prop('checked');
         breadcrumbs = this.elements.breadcrumbs.prop('checked');
         if (fingerprint && elementPath && breadcrumbs && elementFullXPath) {
            matchTypes = 'all';
         } else {
            matchTypes = '';
            if (fingerprint) {
   ...
   enableControls: function () {
      if (!this.context.controlsEnabled) {
         ...
         this.elements.editFullXPath.attr('disabled', 'true');
         this.elements.allInputs.attr('disabled', 'true');
         this.elements.allButtons.button('option', 'disabled', true);
      } else {
         this.elements.editElementPath.removeAttr('disabled');
         this.elements.editFullXPath.removeAttr('disabled');
         this.elements.allInputs.removeAttr('disabled');
...
DejaClickUi.VariableOptionsProperty.prototype = {
   ...
   refresh: function () {
      ...
      this.context.updatePropertyModified(this,this.elements.section,
         [ 'DC_OPTID_LOCATIONID' ]);
      this.state.hasNewValue = false;
      this.enableControls();
   },
...
/**
 * Property allowing configuration of custom cookies.
 * @constructor
 * @implements {DejaClickUi.Property}
 * @param {!DejaClickUi.PropertyContext} aContext The context to which
 *    the property applies.
 */
DejaClickUi.CookiesProperty = function (aContext) {
   this.context = aContext;
   ...
   this.elements = {
      ...
      allButtons: $('#cookiesSection').find('button')
   };
...
DejaClickUi.CookiesProperty.prototype = {
   /** Constructor for objects with this prototype. */
   constructor: DejaClickUi.CookiesProperty,

   /**
    * Shut down the property and release all external objects.
    * @this {DejaClickUi.CookiesProperty}
    */
   close: function () {
      if (this.hasOwnProperty('elements')) {
         this.elements.allButtons.off('click').button('destroy');
    ...
    */
   refresh: function () {
      var select, script, index, cookie, url, name, value, format, summary;

      // Remove any previously added cookies.
      select = this.elements.select;
      select.empty();

      // Find the cookie script elements.
      this.cookies = this.context.element.getElementsByTagName('cookie');

      script = this.context.script;
      index = this.cookies.length;
      while (index !== 0) {
         --index;
         cookie = this.cookies[index];
         url = script.domTreeGetHeaderParam(cookie, 'url');
         if (url == null) {
            url = '';
         }
         name = script.domTreeGetHeaderParam(cookie, 'name');
         if (name == null) {
            name = '';
         }
         summary = url + " - " + name;
   ...
   /**
    * Open a dialog window to define a new custom cookie.
    * @this {DejaClickUi.CookiesProperty}
    * @param {!Event} aEvent A jQuery click event on the add cookie button.
    */
   add: function (aEvent) {
      try {
         this.context.openDialog('ui/content/dejaCookie.html',
            {
               context: this.context.element,
               item: null
            },
   ...
   /**
    * Open a dialog window to edit the selected custom cookie.
    * @this {DejaClickUi.CookiesProperty}
    * @param {!Event} aEvent A jQuery click event on the edit cookie button.
    */
   edit: function (aEvent) {
      var selected, index;
      try {
         ...
         if (selected.length !== 0) {
            index = Number(selected[0].getAttribute('value'));
            if ((0 <= index) && (index < this.cookies.length)) {
               this.context.openDialog('ui/content/dejaCookie.html',
                  {
                     context: this.context.element,
                     item: this.cookies[index]
                  },
                  this.completeEdit.bind(this));
            }
         }
   ...
   /**
    * Refresh the property after a cookie has been added or edited.
    * @this {DejaClickUi.CookiesProperty}
    * @param {?Element} aCookie The added or edited cookie element,
    *    or null if the operation was canceled.
    */
   completeEdit: function (aCookie) {
      try {
         if (aCookie !== null) {
            this.context.syncPropertyChange('cookies');
         }
      } catch (ex) {
         this.context.logger.logException(ex);
      }
   ...
   /**
    * Remove the selected custom cookie.
    * @this {DejaClickUi.CookiesProperty}
    * @param {!Event} aEvent A jQuery click event on the remove cookie button.
    */
   remove: function (aEvent) {
      var selected, index, cookie, cookies;
      try {
         selected = this.elements.select.prop('selectedOptions');
         if (selected.length !== 0) {
            index = Number(selected[0].getAttribute('value'));
            if ((0 <= index) && (index < this.cookies.length)) {
               cookie = this.cookies[index];
               cookies = cookie.parentNode;
               this.context.script.domTreeRemoveNode(cookie);
               if (this.cookies.length === 0) {
                  this.context.script.domTreeRemoveNode(cookies);
               } else {
                  this.context.script.renumberElements('cookie');
               }
            }
            this.context.syncPropertyChange('cookies');
         }
   ...
   completeEdit: function (aBranch) {
      try {
         if (aBranch !== null) {
            ...
            this.context.observerService.notifyLocalObservers('dejaclick:updatetreeview', {});
         }
      } catch (ex) {
         this.context.logger.logException(ex);
      }
...
/**
 * Property controlling the configuration options for JavaScript Dialogs.
 * 
 * @constructor
 * @implements {DejaClickUi.Property}
 * @param {!DejaClickUi.PropertyContext} aContext The context to which
 *    the property applies.
 */
DejaClickUi.JSDialogOptionsProperty = function (aContext) {
   this.context = aContext;
   ...
   this.elements = {
      section: $('#jsDialogOptionsSection'),
      description: $('#jsDialogOptionDescription'),
      injection: $('#jsDialogInjectionEnabled'),
      reset: $('#jsDialogOptionsReset'),
      apply: $('#jsDialogOptionsApply')
   };

   this.elements.injection.on('change', this.changeValue.bind(this));
   this.elements.reset.button().on('click', this.reset.bind(this));
   this.elements.apply.button().on('click', this.apply.bind(this));
};

DejaClickUi.JSDialogOptionsProperty.prototype = {
   /** Constructor for objects with this prototype. */
   constructor: DejaClickUi.BlockOptionsProperty,
   ...
   close: function () {
      if (this.hasOwnProperty('elements')) {
         this.elements.apply.off('click').button('destroy');
         this.elements.reset.off('click').button('destroy');
         this.elements.injection.off('change');
      }
      delete this.elements;
      delete this.state;
      delete this.context;
   ...
   refresh: function () {
      ...
      if ( typeof browser !== "undefined" ) {
         this.elements.description.text(this.context.getMessage('deja_properties_notSupportedAtFirefox'));
         this.elements.injection.prop('checked', false);
      } else {
         this.elements.injection.prop('checked',
         this.context.getPreference('DC_OPTID_JS_DIALOG_INJECTION_ENABLED'));
         this.state.hasValue = this.context.updatePropertyModified(this,
            this.elements.section,
            [ 'DC_OPTID_JS_DIALOG_INJECTION_ENABLED' ]);
         this.state.hasNewValue = false;
      }
      this.enableControls();
   ...
   reset: function (opt_event) {
      try {
         this.context.resetPreference('DC_OPTID_JS_DIALOG_INJECTION_ENABLED');
         this.context.syncPropertyChange('jsDialogOptions');
      } catch (ex) {
         this.context.logger.logException(ex);
      }
   },
   ...
   apply: function (opt_event) {
      try {
         this.context.setPreference('DC_OPTID_JS_DIALOG_INJECTION_ENABLED',
            this.elements.injection.prop('checked'));
         this.context.syncPropertyChange('jsDialogOptions');
      } catch (ex) {
         this.context.logger.logException(ex);
    ...
    */
   enableControls: function () {
      if ( typeof browser !== "undefined" || !this.context.controlsEnabled) {
         this.elements.injection.attr('disabled', 'true');
         this.elements.reset.button('option', 'disabled', true);
         this.elements.apply.button('option', 'disabled', true);
      } else {
         this.elements.injection.removeAttr('disabled');
         this.elements.reset.button('option', 'disabled',
            !this.state.hasValue && !this.state.hasNewValue);
         this.elements.apply.button('option', 'disabled',
            !this.state.hasNewValue);
...
DejaClickUi.NetworkActivityProperty = function (aContext) {
   ...
   this.elements = {
      ...
      warningsWrap: $('#networkWarningsWrap'),
...
DejaClickUi.NetworkActivityProperty.prototype = {
   ...
   refresh: function () {
      ...
      this.elements.warnings.prop('checked',
         this.context.getPreference('DC_OPTID_NETWORKWARNING'));
      ...
      this.context.category === "script" && this.context.isFirefox ? this.elements.warningsWrap.show() : this.elements.warningsWrap.hide();
   ...
   reset: function (opt_event) {
      try {
         ...
         this.context.resetPreference('DC_OPTID_NETWORKWARNING');
   ...
   apply: function (opt_event) {
      ...
      try {
         ...
         this.context.setPreference('DC_OPTID_FULLPAGEOBJECTS',
            this.elements.fullpage.prop('checked'));
         this.context.setPreference('DC_OPTID_NETWORKWARNING',
            this.elements.warnings.prop('checked'));
...
DejaClickUi.MobileOptionsProperty = function (aContext) {
   ...
   this.elements = {
      ...
      description: $('#mobileOptionsSectionDescription'),
