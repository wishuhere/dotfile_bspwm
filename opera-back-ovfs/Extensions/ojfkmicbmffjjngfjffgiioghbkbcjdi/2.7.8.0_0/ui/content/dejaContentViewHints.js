...
DejaClickUi.CVHints.prototype = {
   ...
   init: function () {
      try {
         ...
         DejaClick.service.__modal.setTitle('deja_contentViewHints_title');
         this.initAvailableComponents();
         this.enableControls();
      } catch (ex) {
            console.log('hints_init ' + ex);
   ...
   initAvailableComponents: function () {
      ...
      try {
         ...
         this.componentsTreeInstance = $.jstree._reference(componentsTreeContainer);
            ...
            console.log('hints_initAvailableComponents ' + ex);
            DejaClick.utils.logger.logException(ex);
      }
   },
   ...
   apply: function (aEvent) {
      ...
      try {
         ...
         this.close();
         DejaClick.service.__modal.close(components.length && components || null);
         ...
         if (this.dialog == null) {
            this.dialog = new this.DialogWindow(aUrl,
               ...
               this.closeDialog.bind(this,
                  ((opt_callback == null) ? null : opt_callback.bind(this) )));
            this.enableControls();
         }
      } catch (ex) {
            DejaClick.utils.logger.logException(ex);
      ...
      try {
         ...
         for (i = 0; i < rawUrls.length; i++) {
            try { 
               ...
               //If we have an error processing just one single URL, we can just skip it. Otherwise the ContentViews config screen will be empty
               DejaClick.utils.logger.logWarning("Error processing urls [Invalid URL = "+rawUrls[i]+"]");
               DejaClick.utils.logger.logException(e);
            }
         }
   ...
   function initialize() {
      try {
         DejaClickUi.cvHints = new DejaClickUi.CVHints(
            ...
            // window.broadcastResize(DejaClick.service.__modals[DejaClick.service.__modals.length-1]);
      } catch (ex) {
         DejaClick.utils.logger.logException(ex);
      }
   }
