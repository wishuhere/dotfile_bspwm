...
(function () {
            ...
            chrome.windows.get(windowId, {}, function(aWindow) {
               if (aWindow) {
                  ...
                  if (aWindow.state !== 'normal' && navigator.appVersion.indexOf("Win")=== -1) {
                     ...
                     //UXM-11026 - When switching out of fullscreen/maximized we have to define the height too.
                     if ( aWindow.state == 'fullscreen' || aWindow.state == 'maximized' ) {
                        updateObj.height = aWindow.height;
                     }
                  }
   ...
   function handleBrowserAction(aTab) {
      try {
         if (DejaClick.sidebarWindow === null) {
            ...
            DejaClick.service.windowId = aTab.windowId;
            DejaClick.utils.observerService.notifyObservers('dejaclick:closeproperties', {width:400, resize:false})
   ...
   function onDejaIntegratedModeDetected(aDetails) {
      ...
      if (aDetails.url.indexOf("/cgi-bin/gologin?state=source%3Duxm_integrated_deja") == -1) {
         return;
      }
