...
DejaClick.PreferenceService.prototype = {
   ...
   defineFromFile: function (aId, aDefaultValueFileName, aType, opt_name) {
      ...
      if (chrome && chrome.extension && chrome.runtime.getURL) {
         ...
         xhr.open("GET", chrome.runtime.getURL('/common/data/' + aDefaultValueFileName + '.json'), true);
   ...
   /**
    * Using the preference object it returns the appropriate value 
    * for the received text.
    * 
    * @param {*} aId 
    * @param {*} aValue 
    */
   decodeValue: function (aId, aText) {
      return this.m_prefs[aId].type.decode(aText);
   },

   /**
    * Returns the name of the preference (the name used to store it in the script XML).
    * 
    * UXM-11948 - Adding this so the prefence service can be used for node attributes too, not just for
    * preferences.
    * 
    * @param {*} aId 
    */
   getName: function (aId) {
      return this.m_prefs[aId].name;
   },
   ...
   setPrefOption: function (aId, aValue, opt_script, opt_event, opt_modTag) {
      ...
      if (!pref.type.validate(aValue)) {
         ...
         if (pref.value !== aValue) {
            oldValue = pref.value;
            //UXM-11093 - Firefox garbage collector is more aggresive than Chrome's one
            //I am creating an new object instead of referencing
            //the coming one, as if the object that created this property is 
            //closed this object preference is also removed and 
            //we get "can't access dead object" at the TestOnDemand
            pref.value = JSON.parse(JSON.stringify(aValue)); 
