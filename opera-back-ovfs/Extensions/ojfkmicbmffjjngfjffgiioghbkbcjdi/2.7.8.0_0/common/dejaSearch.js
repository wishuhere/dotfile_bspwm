...
DejaClick.Search.prototype = {
   ...
   getXPath : function( aTargetNode, optimized )
   {
      ...
      while (node && node.nodeType == 1) {  // element types only
         nodePath.push(node);
         if ( optimized && node.nodeType == 1 && node.hasAttribute("id")) {
            break;
         }
      ...
      while (i !== 0) {
         ...
         if (node && node.nodeType == 1) {
            if (optimized && i == nodePath.length - 1 && node.hasAttribute("id")) {
   ...
   processXPath : function( aNode, aXpath )
   {
      ...
      //UXM-10430 - JetBlue full XPath were returning errors due to invalid values. Return empty node list in those cases and log the exception.
      try {
         ...
         this.m_logger.logWarning("Exception while processing XPath "+aXpath+" - "+ex);
      }
...
//                  this.m_logger.logWarning("Invalid HTML detected and mozilla bug #260967 prevents determining parent FORM element.");
   ...
   getCandidateNodesByNameOrID: function( aElementNodes, aScore, aAttribs, aCandidates ) {
      ...
      for (i=0; i < aElementNodes.length; i++) {
         ...
         if ((elemid && elemid == aAttribs.id) ||
            ...
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("found an id or name attrib, setting crumbScore to: " + aScore ); }
            aCandidates.push({
               elem: elem,
               crumbScore: aScore
            });
      ...
      if (aCandidates.length > 1) {
         ...
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("**duplicate id or name attribs found...removing all name/ID-match candidates" ); }
   ...
   followBreadcrumbs : function( aDocNode, aParentNode, aBreadcrumbs, aCrumbIndex, aCurrentScore, aFindNode, aHailMary, aIgnoreNamedAttr )
   {
      var attribs, perCrumbWeight, attribBonus, candidateNodes, duplicateNames,
         ...
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("\n[ [ [ inside followBreadcrumbs ] ] ]"); }
         ...
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("processing tag [" + crumb.tag + "]"); }
         ...
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("count of namedAttribs: " + namedAttribs ); }

         // magic scoring calculations
         perCrumbWeight = (1.0) / aBreadcrumbs.length;
         var xtraAttribs = attribCount - namedAttribs;
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("count of xtraAttribs: " + xtraAttribs ); }
         ...
         var elemid, elemname;
         if (namedAttribs) {
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("trying to find matching TAG using NAME or ID under the PARENT node..."); }
            duplicateNames = this.getCandidateNodesByNameOrID(elementsPar,
               perCrumbWeight * (0.75 + attribBonus), attribs, candidateNodes);

            if (candidateNodes.length === 0) {
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("none found, so trying to find matching TAG using NAME or ID under the DOCUMENT node..."); }
         ...
         if (candidateNodes.length === 0) {
            ...
            if (pNode.nodeName != 'browser' && pNode.nodeName != 'location' && pNode.nodeName != 'document') {
               ...
               if (elem) {
                  ...
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("trying to find matching TAG using element INDEX under the PARENT node [" + pNode.nodeName + "]..."); }
            ...
            if (aFindNode || pNode.nodeName == 'browser' || pNode.nodeName == 'location' || pNode.nodeName == 'document') {
               ...
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("trying to find matching TAG anywhere under its PARENT node [" + pNode.nodeName + "]..."); }
            ...
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("trying to find matching TAG anywhere under its DOCUMENT node..."); }
         ...
         if (candidateNodes.length === 0) {
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("Unable to find any suitable target elements for this crumb.  Resetting breadcrumbsScore."); }
            ...
            if (this.m_logger.tracesearch) { this.m_logger.logDebug(candidateNodes.length + " candidate target element(s) found."); }
            ...
            for (i=candidateNodes.length-1; i>=0 ; i--) {
               ...
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("crumbScore for this candidate node is: " + crumbScore ); }
               // get the highest scoring base crumb (no attribs) and store it away for later
               if (crumbScore >= highestCrumbScore || targetNode == null) {
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("the crumbScore for this candidate is greater than or equal to our previous"); }
                  highestCrumbScore = crumbScore;
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("highestCrumbScore is now set to: " + highestCrumbScore + "\n"); }
               }
               ...
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("checking attributes of candidate node " + cNode ); }
               ...
               for (var attrib in attribs) {
                if (attribs.hasOwnProperty(attrib)) {
                  ...
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug(" testing attribute: " + attrib ); }
                  ...
                  if (attrib == 'id' || attrib == 'name') {
                     ...
                     if (!duplicateNames) {
                        if (this.m_logger.tracesearch) { this.m_logger.logDebug("  skipping attribute scoring for: " + attrib + "=[" + attribs[attrib] + "], matching name or an id score already factored in"); }
                     ...
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("  skipping attribute scoring for: " + attrib + "=[" + attribs[attrib] + "], duplicate name or id attribute previously scored"); }
                        continue;  // skip scoring any matching name/id attrib more than once
                     }
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("  allowing attribute scoring for: " + attrib + "=[" + attribs[attrib] + "], first duplicate name or id attribute scored at a lower value"); }
                        ...
                        if (attrib=='href') {
                           ...
                           if (tempAttrib != attribs[attrib]) {
                              ...
                              if (partialURLScore > largestURLScoreFound) {
                                 ...
                                 if (this.m_logger.tracesearch) { this.m_logger.logDebug("ignoring candidate's pre-assigned crumbScore due to inferior URL match"); }
                                 crumbScore = 0;
                  ...
                  if (tempAttrib == attribs[attrib]) {
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    attribute accepted:    " + attrib + "=[" + attribs[attrib] + "]"); }
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    adding to attribScore: " + perAttribWeight); }
                     attribScore += perAttribWeight;
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    attribScore is now:    " + attribScore ); }

                  } else if (proxyAttribScore > 0) {
                     // alternate text-depiction scoring used in place of attribute scoring
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    proxy attribute matched:    " + attrib + "=[" + attribs[attrib] + "]"); }
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    adding to attribScore: " + proxyAttribScore ); }
                     attribScore += proxyAttribScore;
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    attribScore is now:    " + attribScore ); }

                  } else if (partialURLScore > 0) {
                     // navigation document or location url attrib was partially matched
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    url attribute partially matched:    " + attrib + "=[" + attribs[attrib] + "],  target value=[" + tempAttrib + "]"); }
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    adding to attribScore: " + partialURLScore ); }
                     attribScore += partialURLScore;
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("    attribScore is now:    " + attribScore ); }
                  } else {
                     if (this.m_logger.tracesearch) { this.m_logger.logDebug("  attribute rejected: " + attrib + "=[" + attribs[attrib] + "], does not match candidateNode value of [" + tempAttrib + "]"); }
                  }
                }
               ...
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("candidate total score is: " + candidateScore); }
               ...
               if (candidateScore >= highestCandidateScore && xtraAttribs > 0 && attribScore === 0) {
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("factoring in only 85% of candidate total because no available attributes matched"); }
                  crumbScore *= 0.85;
               ...
               if (candidateScore >= highestCandidateScore || targetNode == null) {
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("***this candidate has a higher score, or has an equal score but is more recent (higher quality match)"); }
                  highestCandidateScore = candidateScore;
                  targetNode = cNode;
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("***highestCandidateScore is now: " + highestCandidateScore ); }
               }
         ...
         if (this.m_logger.tracesearch) {
            this.m_logger.logDebug("followBreadcrumbs - highest base Crumb score found was: " + highestCrumbScore );
            this.m_logger.logDebug("followBreadcrumbs - highest Candidate score found was: " + highestCandidateScore );
         }

         currentScore += highestCandidateScore;
         if (this.m_logger.tracesearch) { this.m_logger.logDebug("cumulative breadcrumbsScore for this target is now: " + currentScore ); }
...
/*
         if (aHailMary && currentScore != 1 && highestCandidateScore < highestCrumbScore) {
            ...
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("highestCandidateScore <= highestCrumbScore, not good enough, resetting targetNode to force a hail mary..."); }
            targetNode = null;
         }
*/
         if (this.m_logger.tracesearch || this.m_logger.debugsearch ) { 
            this.m_logger.logDebug("\nfollowBreadcrumbs scoring complete...unwinding recursively from target tag [" + crumb.tag + "]"); 
            this.m_logger.logDebug("followBreadcrumbs scoring complete - Score: "+currentScore); 
         }
   ...
   searchFingerprint : function ( aDocNode, aMethodData, aPreferredDoc )
   {
         ...
         if (targetNode && breadcrumbs) {
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("a target node was found - beginning fingerprint scoring..."); }
            ...
            if (resultObj.targetNode) {
               ...
               if (targetScore != 1 && aPreferredDoc) {
                  if (this.m_logger.tracesearch) { this.m_logger.logDebug("not a perfect score for preferred doc, clearing targetNode to force hail mary..."); }
                  targetNode = null;  // clearing node forces alt doc scanning
               }
               if (this.m_logger.tracesearch) { this.m_logger.logDebug("final targetScore = " + targetScore); }

            } else {
               targetNode = null;
            }
            if (this.m_logger.tracesearch || this.m_logger.debugsearch ) { this.m_logger.logDebug("\nend of fingerprint scoring - Score: "+targetScore); }

         } else {
            if (this.m_logger.tracesearch || this.m_logger.debugsearch) { this.m_logger.logDebug("searchFingerprint - target node NOT found - returning empty handed"); }
         }
   ...
   searchElementPath : function( aDocNode, aMethodData, aPreferredDoc )
   {
         ...
         if (targetNode) {
            if (this.m_logger.tracesearch) { this.m_logger.logDebug("a target node was found - beginning elementpath scoring..."); }
            ...
            if (this.m_logger.tracesearch || this.m_logger.debugsearch ) { this.m_logger.logDebug("\nend of elementpath scoring - Score: "+targetScore); }

         } else {
            if (this.m_logger.tracesearch || this.m_logger.debugsearch ) { this.m_logger.logDebug("searchElementPath - target node NOT found - returning empty handed"); }
         }
   ...
   /**
    * Calls the corresponding search element function depending on the type 
    * 
    * @param {*} aDocNode 
    * @param {*} aType 
    * @param {*} aMethodData 
    * @param {*} aPreferredDoc 
    */
   searchByType : function ( aDocNode, aType, aMethodData, aPreferredDoc )
   {
      switch (aType) {
         case this.FINGERPRINT:
            return this.searchFingerprint(aDocNode, aMethodData, aPreferredDoc);

         case this.ELEMENTPATH:
         case this.ELEMENTFULLXPATH:
            return this.searchElementPath(aDocNode, aMethodData, aPreferredDoc);
          
         case this.BREADCRUMBS:
            return this.searchBreadcrumbs(aDocNode, aMethodData, aPreferredDoc);
      
         default:
            this.m_logger.logWarning("Invalid search type received: "+aType);
            return null;
      }
   },
    ...
    * @const
    */
   ELEMENTFULLXPATH_TYPE: /fx|all/i,
   ELEMENTFULLXPATH: 'elementfullxpath',
    ...
    * @param {?string} aElementPath Optimized XPath query identifying the target.
    * @param {?string} aElementFullXPath Full XPath query identifying the target.
    * @param {?Array.<!{
    *    tag: string,
    *    index: string,
    ...
    */
   searchForTargetNode: function(aRoot, aFingerprint, aElementPath, aElementFullXPath,
      ...
      if ((aElementPath !== null) && this.ELEMENTPATH_TYPE.test(aMatchTypes)) {
         ...
         result = this.searchElementPath(aRoot,
            params , false);
         result.type = this.ELEMENTPATH;
         result.params = params;
         searchResults.push(result);
      }

      // ElementFullXPath search
      if ((aElementFullXPath !== null) && this.ELEMENTFULLXPATH_TYPE.test(aMatchTypes)) {
         if (this.m_logger.tracesearch) {
            this.m_logger.logDebug('\n---------------------------\n' +
               'beginning elementfullxpath search for target: ' + aElementFullXPath);
         }
         const params = {
            elementpath: aElementFullXPath,
            breadcrumbs: aBreadcrumbs,
            bIgnoreNamedAttr: aIgnoreNamedAttr
         };
         result = this.searchElementPath(aRoot,
            params, false);
         result.type = this.ELEMENTFULLXPATH;
         result.params = params;
         searchResults.push(result);
      }
