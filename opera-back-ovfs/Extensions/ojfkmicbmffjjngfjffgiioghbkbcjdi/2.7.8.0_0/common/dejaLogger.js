...
DejaClick.Logger = function (aListener) {
   ...
   /**
    * Flag indicating whether ALL the debug messages related to DOM search should
    * be logged.
    * (labeled as "trace" as there is a lower log level "debug")
    * @type {boolean}
    */
   this.tracesearch = false;

   /**
    * Flag indicating whether most of important debug messages related to DOM search should
    * be logged.
    * @type {boolean}
    */
   ...
   this.setDebugOptions = function (aOptions) {
      ...
      if (/all/i.test(aOptions)) {
         ...
         self.tracesearch   = /sea/i.test(aOptions);  //DOM search
         self.debugsearch   = /seb/i.test(aOptions);  //DOM search Basic
   ...
   this.logException = function (aError, opt_function, opt_message) {
      ...
      try {
         ...
         if (aError instanceof Error) {
            ...
            //Firefox Quantum UXM-11026
            //Some exceptions are not of type "Error" and 
            //we weren't showing the error message. That's why I added
            //this option trying to get the message and the stack
            ...
            //Firefox Quantum UXM-11026
            //And also this final one... just in case.
            message += aError;
         }
