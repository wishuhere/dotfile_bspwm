...
*/
         ...
         if (gDC._runMode == constants.RUNMODE_REPLAY) {
            ...
            if (!gDC.loadContentViews()) {
               gDC.handleReplayFailure( "dcMessage_errorLoadCVs", null, constants.STATUS_SCRIPT_PARSE_ERROR );
               return;
            }
   ...
   appendRecording : function()
   {
      try {
         ...
         var addSubscript = (gDC.getSystemIntPref('DC_OPTID_APPENDMODE') == constants.DC_APPENDMODE_SUBSCRIPT);
...
*/
   ...
   prepareRecordData : function( aAddSubscript )
   {
      try {
         ...
         if (aAddSubscript) {
            ...
            var i;
            var defaultMode = gDC.getSystemIntPref('DC_OPTID_APPENDMODE') == constants.DC_APPENDMODE_DEFAULT;
         if (defaultMode) {
            
               // Truncate the recorded actions subtree after the current event
               // by discarding all actions that come after the current action
               var nodesToDelete = [];
               if (gDC.actActionNum) {
            ...
            if (gDC.getSystemIntPref('DC_OPTID_APPENDMODE') == constants.DC_APPENDMODE_INSERT) {
   ...
   applyFrozenSettings: function(opt_callback) {
      ...
      disablePopups =
         script.domTreeGetAttribute(root, 'disablepopups') === 'true';
      disableCookies =
         script.domTreeGetAttribute(root, 'acceptcookies') === '2';
      if (!hideCookies) {
         callsLeft -= 0x2;
      }
      if (disablePopups) {
      ...
      chrome.browsingData.remove({}, {
         appcache: script.domTreeGetAttribute(root, 'clearappcache') === 'true',
         cache: script.domTreeGetAttribute(root, 'clearwebcache') === 'true',
         fileSystems: script.domTreeGetAttribute(root, 'clearfilesystems') ===
            'true',
         formData: script.domTreeGetAttribute(root, 'clearformdata') === 'true',
         indexedDB: script.domTreeGetAttribute(root, 'clearindexeddb') ===
            'true',
         localStorage: script.domTreeGetAttribute(root, 'clearlocalstorage') ===
            'true',
         serverBoundCertificates: script.domTreeGetAttribute(root,
            'clearcertificates') === 'true',
         pluginData: script.domTreeGetAttribute(root, 'clearplugindata') ===
            'true',
         passwords: script.domTreeGetAttribute(root, 'clearpasswords') ===
            'true',
         webSQL: script.domTreeGetAttribute(root, 'clearwebsql') === 'true'
      }, onCallCompleted.bind(null, 0x1));
   ...
   findTargetElement: function(aNode, aMinMatchScore, aCallback) {
      ...
      if ((doc != null) && doc.hasAttribute('docId')) {
         if (this.logger.debugsearch) {
            this.logger.logDebug('Dispatching element search ' + searchId +
      ...
      if (searchObj.responsesLeft.length === 0) {
         ...
         if (this.logger.debugsearch) {
            this.logger.logDebug('Dispatched element search ' + searchId +
   ...
   searchForTargetDocument: function(aTarget) {
      ...
      if (this.logger.debugsearch) {
         this.logger.logDebug('   Target Match Score for ' + requestType +
   ...
   searchComplete : function (aDetails, aTab, aDocId)
   {
      ...
      try {
         ...
         if (gDC.logger.debugsearch) {
            gDC.logger.logDebug('Search ' + aDetails.searchId +
         ...
         if (searchObj.preferred) {
            ...
            if (matchFound && (matchScore + searchObj.documentScore >=
               ...
               if (gDC.logger.debugsearch) {
                  gDC.logger.logDebug('Target element FOUND in preferred document using ' +
               ...
               if (gDC.logger.debugsearch) {
                  gDC.logger.logDebug('Element search in preferred document failed. Dispatched search to documents ' +
            ...
            if (matchFound && (gDC.isDocumentIdTracked(aDocId) || aTab.url.indexOf("file:") == 0)) {
               if ((searchObj.matchDetails == null) ||
                  ...
                  searchObj.matchDocId = aDocId;
                  if (gDC.logger.debugsearch) {
                     gDC.logger.logDebug("*** this targetScore was higher or equal but more recent, saving as the new highest score");
                  }
               }
   ...
   /**
    * Send the keystroke input tp the target tab. 
    * @this {!DejaClick.DejaService}
    * @param {object} aResponse from the chrome debugger protocol.
    */
   handleKeyboardEvent : function ( aResponse )
   {
       try {
          if (aResponse) {
          ...
          if (eventNode.getAttribute('type') == 'change' &&
             this.getEventBoolPref('DC_OPTID_USEKEYSTROKES', eventNode)) {
             var eventParams = this._script.getChildWithTag(eventNode, 'eventparams');
             var replayText = this._script.domTreeGetParam(eventNode, "value", "eventparams");
   ...
   dispatchEventToDocument : function (aTabId, aDocId, aSearchId)
   {
      ...
      try {
         ...
         details = {
            ...
            eventparams: this.convertDomParamsToObject(
               this._script.getChildWithTag(eventNode, 'eventparams')),
            variables: varInfo,
   ...
   processJSDialog : function () 
   {
      try {
         ...
         // Navigate to about:blank if it is a new tab/chrome tab
   ...
   getNextEvent : function( aEventNode )
   {
      try {
         ...
         if (!gDC.actEventNodes) { return null; }      // no events loaded, so nothing to replay
         if (!aEventNode) {
            return {
...
*/
         ...
         //  --------
         gDC._setCurrentEvent( gDC.nextEvent.subscriptNum, gDC.nextEvent.num );
         ...
         if (eventType == 'navigate') {
            try {
               ...
               var requestedURL = gDC.retrieveEventParamValue(eventNode, "urlrequested");
               ...
               browserNode = findTargetBrowserNode( eventNode, minMatchScore );
...
//                  gDC.restartReadyTimeout();
         ...
         //  replay 'tabclose' event
         //  --------
         if (eventType == 'tabclose') {
            try {
               gDC._setWaitType( constants.WAITTYPE_INITIALIZING );

               // TabClose events are those in which the user closed a tab manually
               // in some way (ctrl-W, File->Close Tab, etc) and was not a result
               // of web page (scripted) events.
               ...
               if (gDC.logger.debugprocess) { gDC.logger.logDebug("replayNextEvent: dispatching tabclose event..."); }
               gDC._setWaitType( constants.WAITTYPE_DISPATCHING );
               gDC.restartResponseTimeout();
               gDC.pendingEvent = eventType;
               gDC.thinktimeStart = Date.now();

               chrome.tabs.remove(browserNode.tabId);
               return;

            } catch ( e4 ) {
               throw new Error("Failed to process tabclose event: " + e4 );
            }
         }

         //  replay 'tabfocus' event
         //  --------
         if (eventType == 'tabfocus') {
            try {
               ...
               if (gDC.logger.debugprocess) { gDC.logger.logDebug("replayNextEvent: dispatching tabfocus event..."); }
               gDC._setWaitType( constants.WAITTYPE_DISPATCHING );
               gDC.restartResponseTimeout();
               gDC.pendingEvent = eventType;
               gDC.thinktimeStart = Date.now();

               chrome.tabs.update(browserNode.tabId, {active : true, highlighted : true});
               return;

            } catch ( e5 ) {
               throw new Error("Failed to process tabfocus event: " + e5 );
            }
         }
         ...
         //  --------
         if (eventType == 'click' || eventType == 'move' || eventType == 'drag' || eventType == 'focus' ||
               eventType == 'change' || eventType == 'submit' || eventType == 'hover') {
            gDC._setWaitType( constants.WAITTYPE_INITIALIZING );
            gDC.actionEvents++;
...
*/
    ...
    */
   addDOMSearchTargets : function (aEventNode, aData, aTabId)
   {
      try {
...
//            gDC.processNavigation();
   ...
   onCompleted : function (aDetails) {
      try {
         ...
         if (gDC.logger.debugnetwork) { gDC.logger.logDebug("onNavigateCompleted called for url="+aDetails.url+", tab="+aDetails.tabId); }
         ...
         if (aDetails.frameId === 0) {
            ...
            if (gDC._runMode == constants.RUNMODE_REPLAY || gDC._runMode == constants.RUNMODE_PAUSED) {
//               gDC.lastFocusedBrowserObj.id = aDetails.tabId;
               gDC.processNavigation();
            }
...
*/
   ...
   handleReplayFailureFromDocument : function (aMessage, aTab, aDocId) {
      try {
         ...
         this.logException(e, this.DCMODULE + 'handleReplayFailureFromDocument');
      }
   ...
   handleReplayFailure: function (aMessageId, aMessageString,
      ...
      var msgId, messageText, statusLogId;
      try {
         ...
         this.logger.logInfo('Beginning replay failure processing');
...
*/
   ...
   handleReplaySuccess : function( aMessageID, aMessageString, aReplayComplete )
   {
      try {
         ...
         var statusLogID = gDC.logger.logInfo( gDC._messageBundle.getMessage( "dcMessage_replaysuccess" ) + " event (" + gDC.replayedEvents + ")" +
                           " - [" + (gDC.subscriptNum ? "subscript " + gDC.subscriptNum : "main script") + "] event " + gDC.actEventNum +
...
*/
         ...
         gDC._observerService.notifyObservers("dejaclick:runmode", {runMode : gDC._runMode, eventsCaptured : gDC.eventsCaptured} );
         return;
   ...
   _setCurrentEvent : function( aSubscriptNum, aEventNum )
   {
      try {
...
*/
         ...
         case constants.RUNMODE_RECORD:
            ...
            if (gDC._runType == constants.RUNTYPE_RECORDNEW) {
               ...
               if (gDC.recordedEvents === 0) {
                  ...
                  if (gDC.getSystemIntPref('DC_OPTID_APPENDMODE') == constants.DC_APPENDMODE_SUBSCRIPT) {
   ...
   _handlePausedMode : function()
   {
      try {
       ...
       if (gDC.lastPausedNode && gDC.lastPausedNode.breakpoint && gDC.lastPausedNode.breakpointType == constants.BREAKPOINT_RECORD_APPEND ) {
   ...
   onEventTimeout : function ()
   {
      try {
        ...
        if ((this._runMode != constants.RUNMODE_REPLAY) ||
         ...
         //UXM-6951: Do not fail if we have undergone atleast one location change.
         ...
         if (aDontWait) {
            ...
            if (this.getEventBoolPref('DC_OPTID_FAILONTIMEOUT',
               ...
               if ((this.replayedEvents > 0) && !aAlreadyPrompted &&
                  ...
                  // Special-case:  Enable the "fullpage" option for the previous action's
                  // last event, if not already enabled.  We do this because in many cases,
                  // mismatched navigation events are often a result of the previous web
                  // page requiring that the fullpage option be enabled.
                  if (gDC.actActionNum > 1) {
                     var actionNodes = gDC.actTreeRoot.getElementsByTagName('action');
                     var lastActionNode = actionNodes[ gDC.actActionNum-2 ];
                     var actEventNodes = lastActionNode.getElementsByTagName('event');
                     if (actEventNodes.length > 0) {
                        var lastEventNode = actEventNodes[ actEventNodes.length-1 ];
                        // make sure the event has a network hint associated with it
                        if (lastEventNode && gDC._script.domTreeHasReplayHint(lastEventNode, gDC.DC_NETWORKHINT)) {
                           gDC.setEventBoolPref( 'DC_OPTID_FULLPAGEOBJECTS', true, lastEventNode );
                        }
                     }
                  }
   ...
   initValidator : function(aValidator, aValidationNode)
   {
      ...
      try {
         ...
         if (gDC._script.domTreeHasValidateParam(aValidationNode, "varreference")) {         
            ...
            strMatchText = gDC._variable.computeScriptVariable(scriptVarInfo.varName, scriptVarInfo.varValue);
         ...
         if (!strMatchText || strMatchText.length < 1) {
            return 0;
         }
         ...
         switch (Number(gDC._script.domTreeGetValidateParam(aValidationNode, "matchtype"))) {
            ...
            case 1: // Plain text string
               ...
               if (gDC._script.domTreeGetValidateParam( aValidationNode, "matchword" ) == "true") {
...
//         }
   ...
   keywordSearchComplete : function (aResult, aTab, aDocId)
   {
      ...
      try {
         ...
         if (errorType == 1) { // do action if keyword NOT found
            if (keywordFound) {
               if (gDC.logger.debugprocess) {
                  gDC.logger.logDebug("keyword check successful: required keyword item [" + matchText +
               ...
               if (actionType == 1) {
                  ...
                  if (actionType == 2) {
                     ...
                     if (gDC.logger.debugprocess) {
                        gDC.logger.logDebug("wait-for-keyword validation pending...");
                     }

                     // Bug Fix : UXM-942. Add a delay between validation checks.
                     gDC._setTimeout( function(){gDC.processValidations(domNode, gDC.VALIDATIONTYPE_KEYWORD);}, gDC.DC_OPTVAL_VALIDATIONDELAY );
                     return;
                  }
            ...
            if (keywordFound) {
               ...
               if (gDC.logger.debugprocess) {
                  gDC.logger.logDebug("keyword check successful: error keyword item [" + matchText +
    ...
    */
   onKeywordValidationTimeout: function(aId, aText) {
      var message, logId;
      try {
         this.stopValidationTimeout();
        ...
        // select validations of the given type (or all validations of any type)
         ...
         if (errorType == 1) { // do action if result is false
            if (Boolean(result) == true) {
               if (gDC.logger._debugprocess)  gDC.logger.logDebug("Assertion successful: JavaScript text [" + jsText + "] evaluated to " + strResult +
               ...
               if (actionType == 1) {
                  ...
                  // reset the validation sequence back to its original
                  // value and continue waiting (exit the loop)
                  if (gDC.logger._debugprocess)  gDC.logger.logDebug("wait-for-JavaScript validation pending...");
                  // Bug Fix : UXM-942. Add a delay between validation checks.
   ...
   processValidations : function( aDomNode, aProcessValType)
   {
           ...
           var oValidator = {
            ...
            if (targetNodes.length) {
               ...
               if (navDocNode && navDocNode.hasAttribute('docId')) {
                  ...
                  if (actionType != 2) {
                     gDC.restartValidationTimeout(gDC.onKeywordValidationTimeout.
                        bind(gDC, oValidator.id, oValidator.strMatchText, actionType),
                        gDC.getEventIntPref('DC_OPTID_DOCKEYWORDVALTIMEOUT',
                        gDC.actEventNode));
                  }
               }
               else {
               ...
               if (actionType != 2) {
                  gDC.restartValidationTimeout(gDC.onKeywordValidationTimeout.
                     bind(gDC, oValidator.id, oValidator.strMatchText, actionType),
                     gDC.getEventIntPref('DC_OPTID_SEARCHKEYWORDVALTIMEOUT',
                     gDC.actEventNode));
               }
            }
...
//            gDC._setTimeout( function(){gDC._observerService.notifyObservers( gDC.resEventNode, "dejaclick:cancelnetwork", "branchtaken" );}, 100 );
   ...
   setReplayHints : function ( aLastEvent, aHoverObj )
   {
      try {
         ...
         if ((gDC._script.domTreeHasReplayHint(gDC.actEventNode, gDC.DC_NETWORKHINT) &&
              (gDC._script.domTreeHasReplayHint(gDC.actEventNode, gDC.DC_SCRIPTINGHINT) || aLastEvent) &&
              (eventType !== 'tabopen') && (eventType !== 'tabfocus'))) {
            ...
            gDC.setEventBoolPref('DC_OPTID_FULLPAGEOBJECTS', true, gDC.actEventNode);
         }
...
*/
   ...
   checkAutoPauseMode: function( aPauseMode )
   {
      ...
      if ((gDC.getSystemIntPref('DC_OPTID_AUTOREPLAYPAUSE') == aPauseMode) || currentNode.breakpoint) {
         if (currentNode.breakpoint && currentNode.breakpointType == constants.BREAKPOINT_RECORD_APPEND) {
            gDC.lastPausedNode = currentNode;         
            gDC.setSystemBoolPref('DC_OPTID_APPENDMODE', DejaClick.constants.DC_APPENDMODE_INSERT);
         }
         else {
            gDC.setSystemBoolPref('DC_OPTID_APPENDMODE', DejaClick.constants.DC_APPENDMODE_DEFAULT);
         }
   ...
   domTreeInsertAction : function (aNode, aType)
   {
      try {
         if (gDC._runType == constants.RUNTYPE_RECORDAPPEND &&
             gDC.getSystemIntPref('DC_OPTID_APPENDMODE') == constants.DC_APPENDMODE_INSERT) {
...
*/
      ...
      try {
         ...
         for (var i=0; i < aTargets.length; i++) {
            ...
            if (searchType == 'element') {
               var elementpath = gDC._search.getXPath( domNode );
               if (elementpath) {
   ...
   /**
    * Get the information on the script variable(if any) for the specified event node
    * @this {!DejaClick.DejaService}
    ...
    * @return {string} 
    */   
   retrieveEventParamValue : function( nodeAEvent, strAParamName )
   {
      var strReturn = null;
      var varInfo = gDC.retrieveEventVariableInfo(nodeAEvent);
      if (varInfo.varName ) {
         if (!varInfo.varText) {
            strReturn = gDC._variable.computeScriptVariable(varInfo.varName, varInfo.varValue);
            if (strReturn == null) {         
