...
DejaClickUi.VariableToken.prototype = {
   ...
   init: function() {
      ...
      if (token) {
         ...
         this.elements.location.val(token[1]);
      ...
      this.elements.locations.append(optionElt);
   ...
   apply: function (aEvent) {
      ...
      try {
         ...
         token.push(this.elements.location.val());
         ...
         this.window.returnValue = Array.prototype.slice.call(token);
   ...
   updateType: function(aEvent) {
      ...
      if (this.state.type != this.constants.DC_TOKENTYPE_LOCATION) {
         ...
         this.elements.fsParams.children('div#' + this.state.divIds[this.state.type]).show();
