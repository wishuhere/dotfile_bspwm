...
DejaClick.DejaServiceCS.prototype = {
   ...
   DC_OPTID_SIMULATEENTER : "simulateenter",
   DC_OPTVAL_MUTATIONDELAY : 100,

   DCMODULE : "DejaClick.DejaServiceCS.",

   DC_DEFAULT_FILELIST_PREFIX : "dejaFileList_",
   ...
   LAST_EVENTS_QUEUE_SIZE : 5,

   init : function () {
      this._runMode = constants.RUNMODE_INACTIVE;
      this.pendingKeystrokes = false;
      this.enterKeyPressedOnChangeEvent = false;
      this.bypassKeystrokes = false;
      this.keystrokeCount = 0;
      this.checkboxWasClicked = false;
      this.bufferedKeycodes = null;
      this.currentChangeValue = null; //UXM-12024 - To store the value of the change field before the enter happens (in some cases the "keydown" is cleaning the field and that's causing Deja to save an empty value)
      ...
      this.maxMutations = -1; //UXM-10729 - Configuration option to define the limit of mutations to track while recording. (-1 means unlimited)
      this.maxMutationsWarning = false; //UXM-10729 - Flag to log that we have reached the mutations limit just once.
      this.jsDialogDetection = false; //UXM-12230 - New option to disable the code injection to detect dialog prompts
      this.server = false;
      ...
      this.fileLists = new Map(); //UXM-10587 - Define empty file paths on init.
      this.fileListsDate = null;
      this.lastEventsSent = null; //UXM-10777 & UXM-10705
      this.lastTrustedEvents = null;
      this.pendingMouseDownEvent = null;
      this.lastMouseDownEvent = null; //UXM-10777 & UXM-10705
      this.lastKeyStrokesChange = null; //UXM-10777 & UXM-10705
      this.attachStateListeners();
      this.isFirefox = ( typeof browser !== "undefined" ); //UXM-11026
   ...
   begin : function () {
      try {
         ...
         if (this._runMode == constants.RUNMODE_RECORD) {
            ...
            //JavaScript Dialog detection could be disabled.
            //UXM-12230 - New flag included because code injection was breaking customer sites.
            if ( ! this.jsDialogDetection ) {
               this.logger.logInfo("JS Dialog Detection Disabled. Skipping code injection. ");
            } else if ( this.isFirefox ) {
               this.logger.logDebug("JS Dialog Detection is disabled for Firefox Extension. Please use Chrome if you need JavaScript Dialogs detection. ");
            } else if (!document.xmlVersion) { // Check whether the rendered document is XML.
               ...
               if ( document.head ) {
                  //UXM-12230 - Include the script at the head section if available
            ...
            this._variable = new DejaClick.Variable(this.logger);


            //UXM-11437 - Firefox perfomance metrics are obtained using PerformanceObserver.
            if ( this.isFirefox && this.isServer ) {
               //Performance Metrics
               this.performanceObserver = new PerformanceObserver(this.performanceObserverCollector.bind(this));
               this.performanceObserver.observe({entryTypes: ['resource', 'frame', 'navigation']});
            }             
         }
      } catch (e) {
   ...
   /**
    * Function called by PerformanceObserver API when a new performance metric is available.
    * 
    * This function is just used on Server extension and for Firefox, as for Chrome we still use the Chrome
    * debugger to get the network metrics.
    * 
    * Function added with UXM-11437.
    * 
    * @param {*} aData 
    */
   performanceObserverCollector: function(aData) {
      try {
         if (this.logger.debugtimings) {
            this.logger.logDebug("Received list of performance metrics from PerformanceObserver");
         }
         
         var list = [];
         aData.getEntries().forEach(function(entry) {
            var entryData = {};
            ...
            if (this.logger.debugtimings) {
               this.logger.logDebug("Pushed metrics for type '"+entry.entryType+"' and URL "+entryData.name);
            }
            list.push(entryData);
          }, this);

          if (this.logger.debugtimings) {
            this.logger.logDebug("Notifying observers of 'dejaclick:performancemetrics' with a new set of metrics [Size="+list.length+"]");
          }
          this.observerService.notifyObservers( "dejaclick:performancemetrics", list);

      } catch(e) {
         this.logger.logWarning("Unexpected error at performanceObserverCollector: "+e);
      }
   ...
   attachStateListeners : function () {
      try {
         ...
         addDejaListener(this.observerService, 'dejaclick:prepareuploadfile', this.prepareUploadFileLists, this); //UMX-10587
   ...
   attachInputListeners : function()
   {
      try {
         ...
         if (this.inputListeners) {
            //UXM-10159 - BUG - The listener for mousedown was missing when using record append/insert
            if (this._runMode === constants.RUNMODE_RECORD) {
               this.inputListeners.addDomListener(document, 'mousedown', true, this.dispatchDomUIEvent, this);
               this.inputListeners.addDomListener(document, 'mouseup', true, this.dispatchDomUIEvent, this);
            }
            return;
         }
         ...
         this.inputListeners = new DejaClick.EventRegistration().
                  addDomListener(document, "keyup", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "keypress", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "keydown", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "click", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "change", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "submit", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "focus", true, this.dispatchDomUIEvent, this).
                  addDomListener(document, "mouseover", true, this.dispatchDomUIEvent, this);
      ...
      try {
         ...
         if (this.replayListeners == null) {
           this.replayListeners = new DejaClick.EventRegistration().
              ...
              addDejaListener(this.observerService, 'dejaclick:calculatemfavalue', this.calculateMFAChallengeValue, this). //UXM-11947 - Keystrokes support for MFA
   ...
   getTargetElement : function ( evt )
   {
      try {
         ...
         if (evt) {
            ...
            if(dejaShadowDOM.isShadow(evt)){
               eventTarget = dejaShadowDOM.shadowNode(evt);
            }
...
*/
               ...
               //UXM-10729 - New configuration option to define a limit of mutations to track while recording
               //We added this because our code modifies page elements and some sites are watching for changes
               //and that was breaking the site behavior (i.e. chemicalguys.com)
               if ( this.maxMutations >= 0 && this.mutationsCount > this.maxMutations ) {
                  if ( ! this.maxMutationsWarning ) {
                     this.maxMutationsWarning = true;
                     this.logger.logWarning("Reached the maximum number of mutations ["+this.maxMutations+"]. Skipping any action that modifies site elments.");
                  }
                  //UXM-10729 - We could disable the observer, but I think it makes more sense to just stop modifying the site.
                  //this.mutationObservers.removeAllMutationObserver();
               } else {
                  this.maxMutationsWarning = false;
...
*/
   ...
   checkOnClick : function ( aTarget ) {
      ...
      let hasonclick = typeof aTarget.onclick !== null || aTarget.hasAttribute('click');
      let angularonclick = aTarget.hasAttribute('ng-click');
      let jqueryonclick = false;
      if(typeof jQuery !== 'undefined'){
          var selector = dejaShadowDOM.formatSelector(aTarget);
          jqueryonclick = typeof $._data($(selector),'events').click !== 'undefined'
      }
  
      if (!hasonclick && !angularonclick && !jqueryonclick) {
    ...
    *    elementfullxpath: (string|undefined),
    *    matchTypes: string,
   ...
   elementSearch: function (aArgs) {
      var result;
      try {
         if ( this.logger.tracesearch || this.logger.debugsearch ) { this.logger.logDebug("elementSearch - ["+aArgs.searchId+"] New element search."); }
         ...
         if (this.logger.tracesearch) {
            this.logger.logDebug("["+aArgs.searchId+"] Performing elementSearch...");
         }

         delete this.targetElements[aArgs.searchId];
         delete this.searchResults[aArgs.searchId];
         ...
         if ( this.logger.tracesearch ) { this.logger.logDebug("["+aArgs.searchId+"] Info search ["+aArgs.fingerprint+"]["+aArgs.elementpath+"]["+aArgs.elementfullxpath+"]"); }
         ...
         result = this.search.searchForTargetNode(
            ...
            (aArgs.elementfullxpath == null) ? null : aArgs.elementfullxpath,
            aArgs.breadcrumbs,
            aArgs.matchTypes,
         ...
         if (result.targetNode !== null) {
            ...
            this.searchResults[aArgs.searchId] = result;
         }

         if ( this.logger.tracesearch || this.logger.debugsearch ) { this.logger.logDebug("elementSearch - ["+aArgs.searchId+"] Result search ["+result.targetScore+"]["+result.selected+"]["+(result.targetNode !== null)+"]"); }
   ...
   jsObjInsertTargets : function(aTargets)
   {
      try {
         ...
         // Get optimized elementpath for this target ('element' search types only)
         var elementpath = this.search.getXPath( domNode, true );
         if (elementpath) {
            targetNode.elementpath = elementpath;
           if (this.logger.debugprocess) { this.logger.logDebug( "elementpath for event target inserted" ); }
         }

         //Get full Xpath for this target ('element' search types only)
         var elementfullxpath = this.search.getXPath( domNode, false );
         if (elementfullxpath) {
            targetNode.elementfullxpath = elementfullxpath;
           if (this.logger.debugprocess) { this.logger.logDebug( "elementfullxpath for event target inserted" ); }
         }
   ...
   addElementSearchTargets : function( aElemTarget )
   {
      return this.jsObjInsertTargets( { domNode: aElemTarget, searchType: "element" } );
    ...
    */
   captureEvent : function( aEvent, aElemTarget, aActionType, aHoldNotification )
   {
      try {
...
*/
         ...
         if (this.submitObj && this.actEventNode) {
            if (this.logger.debugprocess) { this.logger.logDebug( "captureEvent - ["+aEvent.type+"] Sending dejaclick:addKeyPressParams. New change value: "+this.submitObj.target.value +" [Old="+this.currentChangeValue+"]"); }
         ...
         if(dejaShadowDOM.isShadow(aEvent)){
            var shadowObject = dejaShadowDOM.getShadowObject(aEvent);
            this.jsObjAddEventParam(eventNode,"shadowParameters",shadowObject.queryParams);  
            this.jsObjAddEventParam(eventNode,"shadowParent",shadowObject.parent);
            this.jsObjAddEventParam(eventNode,"shadowPath",shadowObject.indexArray);
            isShadow = true;
         }          
         ...
         if ( ( aActionType=='mousedown' || aActionType=='click' ) && elemTarget instanceof HTMLInputElement && elemTarget.type.toLowerCase()=='image') {
         ...
         if ( ( aActionType=='mousedown' || aActionType=='click' ) && textInputTarget) {
         ...
         //UXM-10616 - Save the sequence of events for playback
         if (aActionType=='click' && this.lastTrustedEvents && this.lastTrustedEvents.length > 1 ) {
            var sequence = [];
            //Assuming max of 4 events (mousedown, mouseup, focus and click)
            var i;
            //Loop in reverse order (first we check the most recent event)
            for (i=1; i<=4; i++ ) {
               if ( this.lastTrustedEvents.length >= i ) {
                  var eventType = this.lastTrustedEvents[this.lastTrustedEvents.length-i].type;

                  //Event already added, so this should belong to previous click. Sequence ready.
                  if ( sequence.includes(eventType) ) { break; }

                  //If we find a focus that points to another element, let's ignore it.
                  if ( eventType == 'focus' && this.lastTrustedEvents[this.lastTrustedEvents.length-i].elemTarget != elemTarget ) {
                     //Focus belong to another event.
                     continue;
                  }

                  //If it is an expected event, add it at the beginning of the sequence.
                  if ( eventType == 'mousedown' || eventType == 'mouseup'
                        || eventType == 'focus' || eventType == 'click' ) 
                  {
                     sequence.unshift(eventType);
                  } else {
                     //Event not related with click. Sequence ready-
                     break;
                  }
               } else {
                  break;
            ...
            //UXM-10982 - If the target element is an input field, we have to force the focus, if it hasn't been recorded
            if ( ! sequence.includes('focus') && textInputTarget ) {
               if ( sequence.includes('mousedown') ) {
                  sequence.splice(sequence.indexOf('mousedown')+1, 0, 'focus');
               } else {
                  sequence.push('focus');
               }
            }

            //If there is a mouseover the same element just before the click elements, let's add it to the sequence.
            if ( this.lastTrustedEvents.length >= i && this.lastTrustedEvents[this.lastTrustedEvents.length-i].type == "mouseover" ) {
               if ( this.lastTrustedEvents[this.lastTrustedEvents.length-i].elemTarget == elemTarget ) {
                  sequence.unshift('mouseover');
               }
            }
            this.logger.logDebug("["+aActionType+"] Recorded sequence of events: "+sequence.toString());
            this.jsObjAddEventParam(eventNode, "sequence", sequence.toString());
         }
         var searchEl = isShadow ? document.querySelector(shadowObject.parent):elemTarget;
         // attach document and element search targets to this event node
         var searchTargets = this.addElementSearchTargets(searchEl);       
         if (!searchTargets) {
	 ...
	 //    this.jsObjAddEventAttribute(eventNode, "optimizedmatch", "false");
          //   this.jsObjAddEventAttribute(eventNode, this.DC_OPTID_USEMATCHTYPES, "ep,fx"); //UXM-10996
         //}
       
         //UXM-10616 - For mousedown we don't send the event to background until we confirm that we don't have a mouseup/click for the same element.
         if ( aHoldNotification ) {
            if ( this.logger.debugprocess ) {
               this.logger.logDebug("Holding notification of "+aActionType+" event. As we will record it depending on future events.");
            }
         } else {
            this.sendEventToBackground(eventNode);
         }
         this.actEventNode = {};
         return eventNode;
   ...
   addHoverEvents : function( aDocument, aHoverEventID )
   {
      try {
         ...
         for (i=sourceID; i < aHoverEventID; i++) {
            ...
            if (!this.captureEvent( hoverObj, hoverObj.target, 'hover', false )) {
...
*/
   ...
   /**
    * Stores the event in the queue of last events and dispatches it
    ...
    */
   dispatchDomUIEvent: function(event) {
      try {
         var type = event.type;

         
         if ( ! this.lastTrustedEvents ) {
            this.lastTrustedEvents = [];
         }
         ...
         if ( event.isTrusted ) {
            ...
            eventObject.elemTarget = this.getTargetElement(event);
            this.lastTrustedEvents.push(eventObject);
            if ( this.lastTrustedEvents.length > this.LAST_EVENTS_QUEUE_SIZE ) { 
               this.lastTrustedEvents.shift();
            }
            if (this.logger.debugprocess) {
               this.logger.logDebug("Adding event to list of trusted events [Current event type= "+type+"]");
            }
         }

         //UXM-10616 - Ignore non trusted clicks while recording (we just need to record real user interactions, not site generated events)
         if ( ! event.isTrusted && this._runMode == constants.RUNMODE_RECORD ) {
            if (this.logger.debugprocess) { this.logger.logDebug( type+" - dispatchDomUIEvent - Discarding event as it has been generated by the customer site code!"); }
            return;
         } else if ( event.isTrusted ) {
            if (this.logger.debugprocess) { this.logger.logDebug( type+" - dispatchDomUIEvent - Processing event as it trusted!"); }
         } else {
            if (this.logger.debugprocess) { this.logger.logDebug( type+" - dispatchDomUIEvent - Processing event although it is not trusted as we are not in record mode."); }
         }


         //UXM-10616 - We record mousedown events, but we just send them to the background if after the mousedown we don't detect
         //a mouseup, focus or click.
         if ( this.pendingMouseDownEvent && type != 'focus' && type != 'click' && type != 'mouseup' ) {
            this.logger.logInfo("Releasing held notification of mousedown event from "+type);
            
            if ( type == 'change' ) {
               //Assuming that the mousedown triggered a change, so let's assume it is a complete mousedown+mouseup.
               this.jsObjAddEventParam(this.pendingMouseDownEvent.eventNode, "sequence", ["mousedown","focus","mouseup","click"].toString());
               this.logger.logDebug("[change] Sequence overwritten: "+["mousedown","focus","mouseup","click"].toString());
               this.lastMouseDownEvent = this.pendingMouseDownEvent;
               //Changing mousedown to click type!
               this.logger.logInfo("Changing type of mousedown to click, as it triggered a change event. Assuming click will come next.");
               this.pendingMouseDownEvent.eventNode.type = 'click';
            }
            this.logger.logInfo("Sending previously recorded mousedown event [Current event type= "+type+"]");
            this.sendEventToBackground(this.pendingMouseDownEvent.eventNode);
            this.pendingMouseDownEvent=null;
         }
         ...
         switch (type) {
            ...
            default:
               this.logger.logFailure('Received unexpected event type: ' + type);
               break;
         }

      } catch (e) {
         this.reportException( e, this.DCMODULE+"dispatchDomUIEvent" );
      }
   },


   /**
    * Just sends a notification of "dejaclick:addEvent" and stores the event in a list of
    * last events notified.
    * 
    * The list will be used to be able to check the last events sent. 
    * 
    * @param {*} eventNode 
    */
   sendEventToBackground: function( eventNode ) {

      try {
         if ( ! this.lastEventsSent ) {
            this.lastEventsSent = [];
         }

         this.lastEventsSent.push(eventNode);
         if ( this.lastEventsSent.length > this.LAST_EVENTS_QUEUE_SIZE ) { 
            this.lastEventsSent.shift();
         }

         this.logger.logDebug("Sending event to background [Type="+eventNode.type+"]");
         this.observerService.notifyObservers("dejaclick:addEvent", eventNode);      
      } catch (e) {
         this.reportException( e, this.DCMODULE+"sendEventToBackground" );
      }
   },
...
*/
            ...
            //UXM-10776 - For non-input fields let's force keystrokes
            if ( ! textInputTarget ) {
               keyboardEvent = true;
            }
...
//                                   (keyTarget instanceof HTMLHtmlElement || keyTarget instanceof HTMLInputElement));
            var enterKeyPressed = (evt.keyCode == this.DOM_VK_RETURN || evt.keyCode == this.DOM_VK_ENTER);
            if (enterKeyPressed) {
               //UXM-12024 - Track the 'Enter' event globaly so we take it in consideration in following keys pressed.
               this.enterKeyPressedOnChangeEvent = true;
...
*/
               ...
               if (keyTarget.form) {
                  for (var i = 0; i < keyTarget.form.elements.length; i++) {
                     if (/submit/i.test(keyTarget.form.elements[i].type)) {
                        ...
                        if ( this.logger.debugprocess ) { 
                           this.logger.logDebug("onBrowserKeypress - ["+evt.type+"] Sending dejaclick:addKeyPressParams. New Change value: "+keyTarget.value +" [Old="+this.currentChangeValue+"]"); 
                        }
            ...
            if (textInputTarget || recordKeystrokes || keyboardEvent) {
               ...
               //UXM-10777 - If we are here it is because it is a text field or requires keystrokes
               //If it is a text field and we have detected key events, we should replay with keystrokes too.
               this.jsObjAddEventAttribute( eventNode, this.DC_OPTID_USEKEYSTROKES, true );
               

               if (textInputTarget || keyboardEvent ) {
                  this.lastKeyCode = evt.keyCode;
               }

               var keyCode = evt.which;
               //UXM-10776 - For input fields the "value" property exists, if undefined, the textContent should be the one having the text of the element.
               var oldValue = keyTarget.value?keyTarget.value:keyTarget.textContent?keyTarget.textContent:"";
               
               //We have to generate a new change event if:
               //  A) If it is the first key pressed detected 
               //  B) After an enter if the value of the element is empty
               if ( ( !this.pendingKeystrokes 
                      || ( this.pendingKeystrokes && this.enterKeyPressedOnChangeEvent && oldValue === "" && ! enterKeyPressed ) )
                      && !(enterKeyPressed && evt.type == 'keyup')) {

                  if ( this.pendingKeystrokes ) { //UXM-12024
                     this.logger.logInfo("Generating new change event. Because of an 'enter' key pressed. Previous change event will require 'Enter' key simulation [Change="+this.currentChangeValue+"]. ");
                     this.jsObjAddEventAttribute( eventNode, this.DC_OPTID_SIMULATEENTER, true );
                     this.observerService.notifyObservers("dejaclick:addKeyPressParams", eventNode);
                     this.currentChangeValue = null;
                  } else {
                     this.logger.logInfo("Generating new change event. First keystroke detected on field.");
                  }

                  //UXM-12024 - The 'onChange' call updates the 'actEventNode' as it could generate 'Hover' events
                  // that was updating the 'eventNode' variable (reference), changing the description of the change to 'Hover'.
                  // To fix that issue I am doing a backup so we use the correct value and restoring it after the change event.
                  var eventNodeBackup = JSON.parse(JSON.stringify(eventNode));
                  this.onChange( evt, true );  // force a new change event at first keystroke
                  this.actEventNode = eventNodeBackup;
               ...
               var nextChar = "";
               if ( enterKeyPressed && (evt.type == 'keydown' || evt.type == 'keypress' || evt.type == 'keyup'  ) && 
                      this.currentChangeValue !== null && this.currentChangeValue !== "" && oldValue === "" ) 
               {
                  //UXM-12024 - Using previously saved value, as in some cases the Enter keydown is clearing the field (i.e. in chats)
                  this.logger.logInfo("["+evt.type+"] Change event will require 'Enter' key simulation. [Value="+this.currentChangeValue+"]");
                  this.jsObjAddEventParam( eventNode, "value", this.currentChangeValue );
                  this.jsObjAddEventAttribute( eventNode, this.DC_OPTID_SIMULATEENTER, true );
               } else if (evt.type == 'keypress') {
                  // get the (printable) char for the key that was pressed
                  nextChar = (evt.charCode !== 0) ? String.fromCharCode(evt.charCode) : "";
                  ...
                  if ( this.logger.debugprocess ) {
                     this.logger.logDebug("["+evt.type+"] Updating change event value to "+newValue +"[Old="+this.currentChangeValue+"]");
                  }
                  this.currentChangeValue = newValue;
                  this.jsObjAddEventParam( eventNode, "value", newValue );
               }
               else if (evt.type == 'keyup' && evt.which < this.UNPRINTABLE_CHAR_RANGE) {
                  if ( this.logger.debugprocess ) {
                     this.logger.logDebug("["+evt.type+"] Received unprintable character. Keeping change event value to "+newValue +"[Old="+this.currentChangeValue+"]");
                  }
                  this.currentChangeValue = oldValue;
                  this.jsObjAddEventParam( eventNode, "value", oldValue );
               }
               ...
               if(dejaShadowDOM.isShadow(evt)){
                  var shadowObject = dejaShadowDOM.getShadowObject(evt);
                  this.jsObjAddEventParam(eventNode,"shadowParameters",shadowObject.queryParams);  
                  this.jsObjAddEventParam(eventNode,"shadowParent",shadowObject.parent);
                  this.jsObjAddEventParam(eventNode,"shadowPath",shadowObject.indexArray);
                  this.jsObjAddEventParam(eventNode,"shadowValue",dejaShadowDOM.shadowNode(evt).value);
               }
               
               this.logger.logDebug("Sending addKeyPressParams "+this.jsObjGetEventParam(eventNode, "keycodes"));
               this.observerService.notifyObservers("dejaclick:addKeyPressParams", eventNode);
               this.actEventNode = {};
   ...
   /**
    * Processes onClick events.
    * 
    * With the changes for UXM-10616 this function also tracks "mouseup" events. 
    * In the past we were ignoring those events, now we are processing them as clicks,
    * as during playback the click is reproduced with 4 events ("mousedown", "focus", "mouseup" and "click").
    * 
    * TODO Nowadays we are not supporting any mousedown that is not behaving as a click, if we want
    * to support drag&drop we will need to add a separate tracking for mousedown events. But, for the moment,
    * to resolve UXM-10616, recording the mousedown events as clicks seems to be a good improvement.
    * 
    * It is called for both replay and record.
    * 
    * @param {*} evt onClick event information
    * @param {*} type type of event
    */
   onClick : function( evt, type )
   {
      var typeDesc = type?type:"click";
...
*/
         ...
         if (!this.docRoot) {
            if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - Unable to get docRoot object, ignoring this event" ); }
            return;  // ignore this event
         }

         if (this.docRoot.baseURI.match(/about:/) || this.docRoot.baseURI.match(/chrome:/)) {
            if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - no baseURI, ignoring this event" ); }
            return;  // ignore this event
         }


         if (this._runMode != constants.RUNMODE_RECORD && this._runMode != constants.RUNMODE_REPLAY && this._runMode != constants.RUNMODE_PAUSED) {
            if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - invalid runMode (1), ignoring this event" ); }
            if (!this.eventsEnabled) {
         ...
         if (this._runMode == constants.RUNMODE_REPLAY || this._runMode == constants.RUNMODE_PAUSED) {
            if (this.screenModeActive) {
               if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - screen event pending, ignoring this event" ); }
               return;  // ignore click event during screen event dispatch
            }
            if (!this.pendingEvent || this.pendingEvent != "click") {
                if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - no pending click event, ignoring" ); }
                return;  // ignore this event
            }
            // otherwise...
         ...
         if (this.pendingKeystrokes && (this.lastKeyCode == this.DOM_VK_UP || this.lastKeyCode == this.DOM_VK_DOWN)) {
            if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - keystrokes pending, ignoring this event" ); }
            this.pendingKeystrokes = false;
            this.enterKeyPressedOnChangeEvent = false;
...
//            this.eventsSkipped.push( 'skipped' );
         ...
         var originalElementTarget = elemTarget;

         //UXM-10616 - We can discard the previously recorded mousedown if the click event points to the same element.
         if ( this.pendingMouseDownEvent && ( this.pendingMouseDownEvent.elemTarget == elemTarget || this.pendingMouseDownEvent.originalElementTarget == elemTarget )  ) {
            //We record the click and ignore the mousedown recorded
            if ( this.logger.debugprocess ) { 
               this.logger.logDebug("Discarding held mousedown event as we have received a "+typeDesc+" pointing to the same element"); 
            }
            this.pendingMouseDownEvent = null;
            this.lastMouseDownEvent = null;
         } else if ( this.pendingMouseDownEvent ) {
            //Otherwise let's send the hold mousedown event, as the click is pointing to a different element.
            this.logger.logInfo("Releasing held mousedown event as we have received a "+typeDesc+" pointing to a different element");
            this.sendEventToBackground(this.pendingMouseDownEvent.eventNode);
            this.pendingMouseDownEvent = null;
            this.lastMouseDownEvent = null;
         }

         //UXM-10705 - We can ignore a click if the last mousedown already generated an event for
         //the same element. This situation happens just when the mousedown triggers a "change" event. 
         if ( typeDesc == 'click' && this.lastMouseDownEvent 
                  && ! this.pendingMouseDownEvent ) 
         {
            this.logger.logInfo("Click event after last mousedown released by change event.");
            if ( this.lastMouseDownEvent.elemTarget == elemTarget ) 
            {
               this.logger.logInfo("Skipping click event after last mousedown released by change event.");
               this.lastMouseDownEvent = null;
               return;
            } else {
               this.logger.logInfo("Cannot skip click event after last mousedown released by change event.");
            }
         }
...
//              (elemTarget.ownerDocument instanceof ImageDocument && (strTargetName == 'BODY' || strTargetName == 'IMG'))) {
            if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - ignoring non-interesting click event (" + elemTarget.nodeName + ")" ); }
            return;
         }
         ...
         if (strTargetName == 'OPTION' || strTargetName == 'SELECT') {
            ...
            if (selectNode.hasAttribute('multiple') || !processOptionClick) {
               if (this.logger.debugprocess) { this.logger.logDebug( typeDesc+" - ignoring OPTION click" ); }
               return;
            }
         }
...
*/
         ...
         if (this._runMode == constants.RUNMODE_RECORD) {
            ...
            if (document.readyState == "loading") {
               if ( this.logger.debugprocess ) { this.logger.logDebug("Ignoring onloading!"); }
               this.blockEvent( evt );
               return;
            }
            ...
            if (elemTarget.id === "jsShowSourceButton" && elemTarget.value === "Tag Found") {
               if ( this.logger.debugprocess ) { this.logger.logFailure("Ignoring jsShowSourceButton!");}
               return;
            }
         
            // Ignore any spurious clicks after the dom load event.
            if (evt.timeStamp - this.domLoadTimestamp < this.IGNORE_TIMEDIFF ) {
               if ( this.logger.debugprocess ) { this.logger.logFailure("Ignoring spurious clicks!"); }  
               return;
            }
            ...
            if ( this.logger.debugprocess ) { this.logger.logDebug("Capture event: "+typeDesc+"["+((type=='mousedown')?type:'click')+"]["+(type == "mousedown")+"]"); }
            //UXM-10616 - For mousedown we just get the capture information, but we don`t notify to background.
            var captureResult = this.captureEvent( evt, elemTarget, (type=='mousedown')?type:'click', type == "mousedown" );
            if ( type == "mousedown" && captureResult ) {
               this.pendingMouseDownEvent = {
                  eventNode: captureResult,
                  elemTarget: elemTarget,
                  originalElementTarget: originalElementTarget
               };
            ...
            //UXM-10616 - I don't think we should change text selection, we should
            //let the site behave as it during recording.
            //In any case, for sure, don't do anything if the click is not trusted (generated by site code)
            //So, with UXM-10616 changes I am adding a validation to apply this changes just for trusted events.
            //Pending to confirm if we should just remove this code.
            if (textInputTarget && evt.isTrusted ) { 
               if (elemTarget.value) {
...
//         this.areWeThereYet();
         ...
         this.reportException( e, this.DCMODULE+"onClick("+typeDesc+")" );
         return;
      }
   },
    ...
    * 
    * While recording it is called on the first keystroke event, in order to save in the script one change event 
    * with all the text typed. Also, while recording is called when a "change" event is detected, but, if we already created
    * a change event due to keystrokes, the change event is ignored.
    * 
    * 
    * 
    * @param {*} evt 
    * @param {Boolean} fromKeyboardEvent - Just defined when the function is called to create a change event after keyboard events.
    */
   onChange : function( evt, fromKeyboardEvent )
   {
      var eventNode;
      try {
/*
...
*/
         ...
         if (this._runMode == constants.RUNMODE_RECORD && this.submitObj && this.actEventNode) {
            if ( this.logger.debugprocess ) {
               this.logger.logDebug("["+evt.type+"] Submit event. Updating change event value to "+this.submitObj.target.value);
            }
            this.jsObjAddEventParam( this.actEventNode, "value", this.submitObj.target.value );
            this.observerService.notifyObservers("dejaclick:addKeyPressParams", this.actEventNode);
         }
         
         if (this.pendingKeystrokes && ! this.enterKeyPressedOnChangeEvent) {
            if (this.bypassKeystrokes) {

               if (this.logger.debugprocess) { this.logger.logDebug( "onChange ["+evt.type+"] - keystroke recording was bypassed, capturing final event value here: "+evt.target.value ); }
         ...
         } else if ( this.enterKeyPressedOnChangeEvent && this.pendingKeystrokes ) {
            this.logger.logInfo("onChange ["+evt.type+"] - Generating change event because new keys were pressed after hitting enter.");
         }
...
*/
         ...
         //UXM-10777 - Ignore change events when we have already recorded the sequence of keys pressed.
         if ( this._runMode == constants.RUNMODE_RECORD && ! fromKeyboardEvent && this.lastKeyStrokesChange && this.lastKeyStrokesChange.elemTarget == elemTarget ) {
            if ( this.logger.debugprocess ) {
               this.logger.logDebug("Ignoring change event, as we already recorded keystrokes for it.");
            }
            this.lastKeyStrokesChange = null;
            return;
         }

         var inputType = (elemTarget.type)?elemTarget.type.toLowerCase():null;
...
*/
            ...
            if ( typeof browser !== "undefined" ) {
               //TODO Firefox Quantum UXM-11026 - Keystrokes not supported, so we have to record "keyboard" events as changes.
               eventNode.type = "change";
            } else {
               eventNode.type = inputType?"change":"keyboard"; //UXM-10776 - New keyboard event to record&replay key events to non-input fields.
            }  
            var strTargetName = elemTarget.nodeName;
            if (strTargetName) {
               strTargetName = strTargetName.toUpperCase();
            ...
            if ( strTargetName == 'SELECT' ) {
               ...
               var inputType = elemTarget.type.toLowerCase();
               if ( inputType == 'file' ) {
                  this.logger.logInfo("[input file event] Detected a new list of files to upload.");

                  // generate the name for the new filelist
                  var fileListName = String(this.DC_DEFAULT_FILELIST_PREFIX) + String((new Date()).getTime());

                  //Get file content and send to background dejaService.js (will be done async)
                  this.importFilesContent(elemTarget.files, fileListName);

                  //Add the reference name to the event so we can recover file on playback.
                  this.jsObjAddEventParam(eventNode, "filelistref", fileListName);

                  //UXM-10776 - Intralinks upload issue detected while recording with keystrokes enabled.
                  //Input files shouldn't be updated with Keystrokes. Forcing to "false"
                  this.jsObjAddEventAttribute( eventNode, this.DC_OPTID_USEKEYSTROKES, false );
               }
            } else if ( inputType && elemTarget.value ) {
               if ( this.logger.debugprocess ) {
                  this.logger.logDebug("["+evt.type+"] Input type ["+inputType+"]. Setting change event value to "+elemTarget.value);
               }
               this.jsObjAddEventParam(eventNode, "value", elemTarget.value);
            } else {
               //UXM-10766 - For non-input fields let's get the value of the key pressed
               // get the (printable) char for the key that was pressed
               var char = (evt.charCode !== 0) ? String.fromCharCode(evt.charCode) : "";
               if ( this.logger.debugprocess ) {
                  this.logger.logDebug("["+evt.type+"] Non-input type ["+inputType+"]. Setting change event value to "+char);
               }
               this.jsObjAddEventParam(eventNode, "value", char);
            }

            var searchElement = dejaShadowDOM.isShadow(evt) ? evt.target : elemTarget;
            var searchTargets = this.addElementSearchTargets( searchElement );
...
*/
            ...
            //UXM-10777 - We save the last time that a new change event was created based on keystrokes.
            //Then, if we detect a change event, we ignore it, as we already created one.
            if ( fromKeyboardEvent ) {
               this.lastKeyStrokesChange = {
                  eventNode: eventNode,
                  elemTarget: elemTarget
               };
            }
...
*/
   ...
   /**
    * Recovers the content of files and send them to the background.
    * 
    * The background will include them in the script so they can be used on playback
    * 
    * @param {FileList} aFileList List of File objects (as obtained from input file fields).
    * @param {String} aFileListName File list name (to link it with the event)
    * 
    */
   importFilesContent : function( /*nsIFile[]*/ aFileList, /*string*/ aFileListName )
   {
      try {

         function onFileLoadedFn (aFile) {
            return function (evt) {
               aFile.data = evt.target.result;
               DejaClickCs.observerService.notifyObservers("dejaclick:newUploadedFile", aFile);
            };
         }

         function onFileErrorFn (aFile) {
            return function (evt) {
               DejaClickCs.logger.logWarning( "importFilesContent - Unable to import '" + aFile.name
                  + "' due to FileReader error: " + evt.target.error );
               DejaClickCs.reportException( evt.target.error, this.DCMODULE+"importFilesContent" );
            };
         }

         
         // asynchronously read in each file's data and send it to background.
         for (var i=0; i < aFileList.length; i++) {
            var file = {};
            file.fileListName = aFileListName;
            file.name = aFileList[i].name;
            file.type = aFileList[i].type;

            //If file size > 2Mb. Error!!
            if ( ((aFileList[i].size)/1024/1024) > 5 ) {
               this.observerService.notifyObservers("dejaclick:recordfailure", "deja_record_uploadfile_toobig"); 
            } else {
               var reader = new FileReader();
               reader.onload = onFileLoadedFn( file );
               reader.onerror = onFileErrorFn( file );
               reader.readAsDataURL(aFileList[i]);
            }
         }

      } catch ( e ) {
         this.reportException( e, this.DCMODULE+"importFilesContent" );
      }
   },
    ...
    * Feature - UXM-10587
    * 
    * Processes the file lists received from the background. 
    * 
    * Converts all the file lists into DataTransfer objects that
    * can be used to update the file input fields.
    * 
    * @param {Object} data Object with the date of the last update of the lists and the file lists.
    */
   prepareUploadFileLists : function( data ) {
      try {

         var aFileListsDate = data.listsdate;
         var aFileLists = data.filelists;

         if ( this.fileListsDate && this.fileListsDate == aFileListsDate ) {
            //We already have the latest version, nothing to do here.
            return;
         }

         for (var i = 0; i < aFileLists.length; i++) { 
            let dT = new DataTransfer(); // More info at: https://stackoverflow.com/questions/47119426/how-to-set-file-objects-and-length-property-at-filelist-object-where-the-files-a
            this.fileLists.set(aFileLists[i].name, dT);

            function onBlobRead(aBlob, aFileName, aFileType, aDT) {
               let file;
               if ( aFileType ) {
                  file = new File([aBlob], aFileName, { type: aFileType }); 
               } else {
                  file = new File([aBlob], aFileName); 
               }
               aDT.items.add(file);
            };

            for (var j = 0; j < aFileLists[i].files.length; j++) { 
               let fileObject = aFileLists[i].files[j];
               fetch(fileObject.data).then(res => res.blob()).then(res => onBlobRead(res, fileObject.name, fileObject.type, dT ));
            }
         }

         this.fileListsDate = aFileListsDate;

      } catch ( e ) {
         this.reportException( e, this.DCMODULE+"prepareUploadFileLists" );
      }
...
*/
         ...
         //UXM-10616 - We can ignore the focus event if it points to the last mousedown recorded.
         //Otherwise we should send the mouse down.
         if ( this.pendingMouseDownEvent && ( this.pendingMouseDownEvent.elemTarget == elemTarget || this.pendingMouseDownEvent.originalElementTarget == elemTarget )  ) {
            this.pendingMouseDownEvent.hasFocus = true;
            return;
         } else if ( this.pendingMouseDownEvent ) {
            this.logger.logInfo("Releasing held notification of mousedown event from focus.");
            this.sendEventToBackground(this.pendingMouseDownEvent.eventNode);
            this.pendingMouseDownEvent = null;
            this.lastMouseDownEvent = null;
         }
...
*/
         ...
         if (this._runMode == constants.RUNMODE_RECORD) {
            ...
            var searchTargets = this.addElementSearchTargets( elemTarget );
            // attach document and element search targets to this event node
            if (!searchTargets) {
               // In rare cases, if we are unable to insert our targets,
               // it may lead to displaying <???> as the event target in
...
*/
            ...
            if ( this.logger.debugprocess ) {
               this.logger.logDebug("onSubmit - Setting change event value to "+elemTarget.value);
            }
            this.jsObjAddEventParam(eventNode, "value", elemTarget.value);
...
*/
         ...
         if ((this.docRoot !== null) &&
            ...
            if (target !== null) {
               ...
               if ((tag === 'EMBED') ||
                  ...
                  //UXM-10616 - Redirecting to "onClick" to track this event.
                  this.onClick(evt, 'mousedown');
               }
            }
         }
      } catch (ex) {
    ...
    * 
    * For the moment this function just updates the sequence of events related with the last mousedown.
    * If the mouseup points to the same element.
    * 
    * @this {!DejaClick.DejaServiceCS}
    * @param {!Event} evt A mouseup event.
    */
   onMouseUp: function (evt) {
      try {
         if ( this.pendingMouseDownEvent && (this.docRoot !== null) && 
            this._runMode === constants.RUNMODE_RECORD ) 
         {
            var elemTarget = this.getTargetElement(evt);
            if ( elemTarget && 
               ( this.pendingMouseDownEvent.elemTarget == elemTarget || this.pendingMouseDownEvent.originalElementTarget == elemTarget ) 
               && this.lastTrustedEvents && this.lastTrustedEvents.length > 1 ) 
            {
               var sequence = [];
               //Assuming max of 3 events (focus, mousedown, mouseup)
               var i;
               for (i=1; i<=3; i++ ) {
                  if ( this.lastTrustedEvents.length >= i ) {
                     var eventType = this.lastTrustedEvents[this.lastTrustedEvents.length-i].type;
                     ...
                     //UXM-10705 - Assuming that the change was triggered by the click, ignoring
                     if ( eventType == 'change' ) {  
                        continue;
                     }
   
                     //If it is an expected event, add it to the sequence.
                     if ( eventType == 'mousedown' || eventType == 'mouseup'
                           || eventType == 'focus' ) 
                     {
                        sequence.unshift(eventType);
               ...
               this.logger.logDebug("[onMouseUp] Sequence generated at mouseup: "+sequence.toString());
               this.jsObjAddEventParam(this.pendingMouseDownEvent.eventNode, "sequence", sequence.toString());
            }

         }

      } catch (ex) {
         this.reportException(ex, this.DCMODULE + 'onMouseUp');
      }
   },

   /**
    * Sends a "dejaclick:contentscriptlogtrace" message to the background
    * so the log trace is logged there.
    * 
    * This feature is just required for DejaClick server, where the log traces
    * of the content script are lost.
    * 
    * UXM-11863 - New feature to improve visibility of what's happening at the content scripts.
    * 
    * @param {*} aLogLevel 
    * @param {*} aLogMessage 
    */
   logTraceToBackground : function(aLogLevel, aLogMessage) {
      try {
         this.observerService.notifyObservers(
            'dejaclick:contentscriptlogtrace', 
            {
               level: aLogLevel, 
               message: aLogMessage
            }
         ...
         if ( this.logger.debugprocess ) {
            this.logger.logDebug("Sent log trace to background ["+aLogLevel+"]["+aLogMessage+"]");
         }
      } catch ( e ) {
         this.logger.logWarning("sendLogTraceToBackground - Unexpected error sending log trace: "+e);
      }
   },
   ...
   {
      var targetElement, searchResult;
      ...
      try {
         ...
         if(typeof aEventInfo.eventparams.shadowParameters !== 'undefined'){
            var shadowReference = dejaShadowDOM.shadowReference({
               "parent":document.querySelector(aEventInfo.eventparams.shadowParent),
               "queryParams":aEventInfo.eventparams.shadowParameters,
               "indexArray":aEventInfo.eventparams.shadowPath
            });
            shadowReference["isShadow"] = true;
            isShadow = true;
            if(aEventInfo.eventparams.shadowValue !== undefined){
               aEventInfo.eventparams.value = aEventInfo.eventparams.shadowValue;
            }
         } 
         // if we dont have a target node to dispatch to, return;
         if (!this.targetElements.hasOwnProperty(aEventInfo.searchId)) {
            let logMessage = "dejaclick:dispatchevent notification discarded. Search ID "+aEventInfo.searchId+" not found at this document!";
            this.logTraceToBackground(constants.LOG_WARNING,logMessage);
            this.observerService.notifyObservers(
               'dejaclick:dispatchAck', 
               {
                  accepted: false, 
                  error: false,
                  message: logMessage
         ...
         searchResult = this.searchResults[aEventInfo.searchId];

         if (this.logger.debugprocess) {
            this.logger.logDebug( "Processing Events" );
         }

         targetElement = ! isShadow ? targetElement : shadowReference;

         var isClick = aEventInfo.type === 'click' && typeof targetElement.click !== 'undefined';
         var isShadowElementClick = isShadow && isClick;
         
         if(isShadowElementClick){
            targetElement.click();
            this.observerService.notifyObservers('dejaclick:dispatchComplete', {});
            return;
         }
         ...
         if ( this.logger.debugpending ) {
            this.logTraceToBackground(constants.LOG_DEBUG,"dejaclick:dispatchevent notification received. Processing event "+aEventInfo.type);
         }
         ...
         case 'move':
         case 'drag':
            this.replayClickEvent(targetElement, aEventInfo, searchResult); //UXM-12197 - Store search info, so we can revalidate the target during the replay of the browser events.
            break;

         case 'change':
            this.replayChangeEvent(targetElement, aEventInfo, searchResult); //UXM-12197 - Change events may require the same validation of target done for click events!!
            break;
         ...
         default:
            this.logTraceToBackground(constants.LOG_WARNING,"dejaclick:dispatchevent notification discarded. Attempt to replay unknown/unexpected event type: "+aEventInfo.type);
            this.logger.logFailure('Attempt to replay unknown/unexpected event type: ' +
               aEventInfo.type);
            break;
         }
         ...
         this.observerService.notifyObservers(
            'dejaclick:dispatchAck', 
            {
               accepted: true, 
               error: false,
               message: "dejaclick:dispatchevent notification received. Event "+aEventInfo.type+" enqueued."
            }
         );
         
      } catch ( e ) {
         let logMessage = "dejaclick:dispatchevent notification discarded. Unexpected error at processEvents: "+e;
         this.logTraceToBackground(constants.LOG_WARNING,logMessage);
         this.observerService.notifyObservers(
            'dejaclick:dispatchAck', 
            {
    ...
    */
   replayClickEvent: function (aTarget, aEvent, aSearchResult) {

      try {
         if ( this.logger.debugpending ) {
            this.logTraceToBackground(constants.LOG_DEBUG,"replayClickEvent - Starting to process click event. ");
         } 
         ...
         //UXM-10616 - Reproduce exactly the recorded sequence of events (if not recorded use default)
         var sequence = aEvent.type=='mousedown'?["mousedown"]:["mousedown", "focus", "mouseup", "click"];
         if ( aEvent && aEvent.eventparams && aEvent.eventparams.sequence ) {
            var seqArray = aEvent.eventparams.sequence.split(",");
            if ( seqArray && seqArray.length > 1 ) {
               sequence = seqArray;
            }
         }
         ...
         for (var i=0; i < sequence.length; i++) {
            if ( sequence[i] == "focus" ) {
               ...
               queue.push(this.createMouseEvent(sequence[i], aEvent));
            }
         }

         if ( ! queue.includes('mouseover') && !this.NON_MOUSEOVER_ELEMENTS.hasOwnProperty(aTarget.nodeName) ) {
         ...
         this.dispatchState = {
            target: aTarget,
            searchResult: aSearchResult, //UXM-12197 - Search info. Used to revalidate the target during dispatch of browser events.
            event: aEvent,
            queue: queue,
            initialQueueLength: queue.length,
            delay: attrs.hasOwnProperty('dispatchDelay') ? attrs.dispatchDelay : 0,
            isShadow:typeof aTarget.isShadow !== 'undefined'? aTarget.isShadow : false
         };
         this._setTimeout(this.dispatchEvents.bind(this), this.dispatchState.delay);

         if ( this.logger.debugpending ) {
            this.logTraceToBackground(constants.LOG_DEBUG,"replayClickEvent - Dispatched click event with "+queue.length+" events in the queue.");
         } 

      } catch( e ) {
         this.logTraceToBackground(constants.LOG_FAILURE,"replayClickEvent - Unexpected error processing click event: "+e);
         throw e;
      } 
   },
    ...
    * }} aEvent Details of the event to be replayed.
    */
   replayChangeEvent: function (aTarget, aEvent, aSearchResult) {
      // @todo Push aTarget onto eventsTracked?
      ...
      var simulateEnter = (attrs.hasOwnProperty('simulateenter')) ? attrs.simulateenter : false;

      // Remap the event type based on the event details.
      aEvent.type = this.getChangeType(aTarget, aEvent);
      if (aEvent.type == null) {
         this.logTraceToBackground(constants.LOG_WARNING,"Replay failure at Content Script. Replay Change Event. Target Element not found. Returning ST97");
      ...
      if ((aEvent.type == "changetext") || (aEvent.type == "changehtml")) {
         ...
         if (attrs.usekeystrokes) {
            ...
            if (params.keycodes && !params.varreference) {
               ...
               textValue = this.calculateChangeValue(aEvent, params.value);
            ...
            if ( simulateEnter && ! tabRequired ) {
               this.logger.logInfo("Simulating 'Enter' key after replaying change event using browser tab keystrokes.");
               queue.push(this.createKeyboardEvent('keydown', aEvent, '['+this.DOM_VK_ENTER+']'));		
               queue.push(this.createKeyboardEvent('keypress', aEvent, '['+this.DOM_VK_ENTER+']'));            
               queue.push(this.createKeyboardEvent('keyup', aEvent, '['+this.DOM_VK_ENTER+']'));
            } else if (!keyCodes && screenEvent && tabRequired) {
            ...
            if ( simulateEnter ) {
               ...
               this.logger.logInfo("Simulating 'Enter' key after replaying change event.");
               queue.push(this.createKeyboardEvent('keydown', aEvent, '['+this.DOM_VK_ENTER+']'));		
               queue.push(this.createKeyboardEvent('keypress', aEvent, '['+this.DOM_VK_ENTER+']'));            
               queue.push(this.createKeyboardEvent('keyup', aEvent, '['+this.DOM_VK_ENTER+']'));
            } else {
         ...
         queue.push(this.createHtmlEvent('change', aEvent));
         queue.push(this.createFocusEvent('blur', aEvent));
      } else if (aEvent.type == "changefile") {
         // ::::::::::::::::::::::::::::::::::::::::::
         // :: handle file input field - UXM-10587  ::
         // ::::::::::::::::::::::::::::::::::::::::::
         //UXM-10587 - Generate a change event to update the input field 
         queue.push(this.createHtmlEvent('change', aEvent));
      }
      ...
      this.dispatchState = {
         ...
         initialQueueLength: queue.length,
         delay: dispatchDelay,
         isShadow:typeof params.shadowParent !== 'undefined',
         shadowValue:typeof params.shadowValue !== 'undefined' ? params.shadowValue:""
      };
      this._setTimeout(this.dispatchEvents.bind(this), this.dispatchState.delay);
   },

   /**
    * Used to calculate the MFA security question value.
    * 
    * It is used when the replay of a change happens at this content script and also
    * when the replay is with keystrokes and the value is requested through 'dejaclick:calculatemfavalue'
    * 
    ...
    * @param {*} defaultValue 
    */
   calculateMFAsecurityQuestionValue( mfaInfo, defaultValue ) {
      try {
         if ( mfaInfo && mfaInfo.type === 'securityquestions' && mfaInfo.value && mfaInfo.value.length >0 ) {

            this.logger.logInfo("calculateMFAsecurityQuestionValue - Obtaining value from MFA security questions...");
            for (let index = 0; index < mfaInfo.value.length; index++) {
               const questionInfo = mfaInfo.value[index];
               //Step 1 - Validate question
               let keywordFound = this.documentRegexKeywordSearch(questionInfo.keyword.oRegExText, questionInfo.keyword);
               
               //Step 2 - If matches, return the configured value
               if ( keywordFound ) {
                  this.logger.logInfo("calculateMFAsecurityQuestionValue - MFA Question matched. Using answer: "+questionInfo.value);
                  mfaInfo.result = questionInfo.value;
                  mfaInfo.error = false;
                  return mfaInfo;
               }
            }

            this.logger.logWarning("calculateMFAsecurityQuestionValue - No question matched the current page. Returning default value: "+defaultValue);
            mfaInfo.result = defaultValue;
            mfaInfo.error = "No question matched the current page";
         } else {
            this.logger.logInfo("calculateMFAsecurityQuestionValue - Invalid MFA info. Returning default value: "+defaultValue);
            mfaInfo.result = defaultValue;
            mfaInfo.error = "Invalid MFA information";
         }
      } catch ( e ) {
         this.reportException( e, this.DCMODULE+"calculateMFAsecurityQuestionValue" );
         mfaInfo.result = defaultValue;
         mfaInfo.error = "Exception processing MFA information: "+e;
      }
   ...
   /**
    * Called from dejaService ('dejaclick:calculatemfavalue') to calculate the value of MFA security questions.
    * 
    * UXM-11947 - Keystrokes support for MFA
    * 
    * @param {*} mfaInfo 
    */
   calculateMFAChallengeValue: function( mfaInfo ) {
      try {
         if ( mfaInfo && mfaInfo.type === 'securityquestions' && mfaInfo.value && mfaInfo.value.length >0 ) {
            this.logger.logInfo("calculateMFAChallengeValue - Received request from dejaSerivce - Calculating MFA security question value.");
            mfaInfo = this.calculateMFAsecurityQuestionValue(mfaInfo, null);
         } else {
            this.logger.logInfo("calculateMFAChallengeValue - Invalud MFA info. Returning 'null' value");
            mfaInfo.result = null;
            mfaInfo.error = "Invalud MFA info. Returning 'null' value.";
         }
         this.observerService.notifyObservers("dejaclick:calculatemfavaluecomplete", mfaInfo);
      } catch ( e ) {
         this.logTraceToBackground(constants.LOG_WARNING,"calculateMFAChallengeValue - Unexpected error: "+e);
         this.reportException( e, this.DCMODULE+"calculateMFAChallengeValue" );
      }
   },

   /**
    * Calculates the value for change event, from MFA security questions or script variable options.
    * 
    * If there is no MFA or script variable definition returns the default value.
    * 
    * Returns null if there is an error while processing the script variable or MFA.
    * 
    * UXM-11786 - Just for MFA security questions, other futrue MFA features like sms or email won't be 
    * executed from the content script.
    * 
    * @param {*} aEvent 
    * @param {*} defaultValue 
    */
   calculateChangeValue: function (aEvent, defaultValue) {
      try {
      
         var varInfo = aEvent.variables;
         var mfaInfo = aEvent.mfa;
         var replayCount = aEvent.replayCount;
         var replayLocation = aEvent.replayLocation;

         var strText = undefined;
         
         if ( mfaInfo && mfaInfo.type === 'securityquestions' && mfaInfo.value && mfaInfo.value.length >0 ) {
            return this.calculateMFAsecurityQuestionValue(mfaInfo, defaultValue);
            ...
            if (varInfo.varText) {
               ...
               if (strText == null) {
                  this.logTraceToBackground(constants.LOG_WARNING,"Replay failure while getting change script variable value. Returning ST97");
         ...
         this.reportException( e, this.DCMODULE+"calculateChangeValue" );
         return null;
      }
   },
   ...
   getChangeType: function (aTarget, aEvent) {
      try {
         ...
         // UXM-10587 - if the event recorded a file list, the target must be a file input
         if (params && params.filelistref) {
            return "changefile";
         }
   ...
   dispatchEvents: function () {
      var target, searchResult, queue, dejaEvent, params, domEvent, eltList, dispatchDetails;
      ...
      try {
         ...
         if (queue.length === 0) {
            ...
            this.varName = null;
            if ( this.logger.debugpending ) {
               this.logTraceToBackground(constants.LOG_DEBUG,"dispatchEvents - Dispatch done! Notifying dispatchComplete.");
            }
            return;
         }   

         var isShadowElement = this.dispatchState.isShadow;

         target = this.dispatchState.target;
         searchResult = this.dispatchState.searchResult;
         dejaEvent = this.dispatchState.event;
         params = /** @type {!{value:string,submitmode:string}} */
            (dejaEvent.eventparams);
         domEvent = queue.shift();
         if ( this.logger.debugpending ) {
            this.logTraceToBackground(constants.LOG_DEBUG,"dispatchEvents - Processing next element in queue [Type="+domEvent.type+"][Remaining_events_in_queue="+queue.length+"]");
         }
         this.pendingEvent = dejaEvent.type;

         //UXM-12197 - Revalidate the target if the search info is available.
         // The search result is stored JUST for click events, we may want to add it to the change events too.
         if ( searchResult && !isShadowElement) {
            if ( this.logger.debugpending ) {
               this.logger.logDebug("dispatchEvents - [Type="+domEvent.type+"] Revalidating that target still exists. ");
            }
            var targetValidated = false;
            if ( searchResult.selected && searchResult.methodResults ) {
               for (let index = 0; index < searchResult.methodResults.length; index++) {
                  const result = searchResult.methodResults[index];
                  if ( result.type == searchResult.selected ) {
                     if ( this.logger.debugpending ) {
                        this.logger.logDebug("dispatchEvents - [Type="+domEvent.type+"] Revalidating target using "+result.type);
                     }
                     targetValidated = this.search.searchByType(document, result.type, result.params, false);
                     break;
                  }
               }
            }
            ...
            if ( targetValidated ) {
               if (targetValidated.targetScore === 0 || ! targetValidated.targetNode ) {
                  const message = `dispatchEvents - [Type=${domEvent.type}][SearchType=${searchResult.selected}] Target NOT found while revalidating the search. Skipping event.`;
                  this.logger.logWarning(message);
                  this.logTraceToBackground(constants.LOG_WARNING, message);
                  revalidationFailed = true;
               } else {
                  if ( this.logger.debugpending ) {
                     this.logger.logDebug("dispatchEvents - [Type="+domEvent.type+"] Updated target element  after successful revalidation search.");
                  }
                  target = targetValidated.targetNode;
               }
            } else {
               const message = `dispatchEvents - [Type=${domEvent.type}][SearchType=${searchResult.selected}] Target search FAILED while revalidating the search, no result obtained. Skipping event.`;
               this.logger.logWarning(message);
               this.logTraceToBackground(constants.LOG_WARNING, message);
               revalidationFailed = true;
            }
         ...
         if ( ! revalidationFailed ) {
            ...
            if (domEvent.type == 'change' && dejaEvent.type == 'changetext') {
               ...
               var strText = this.calculateChangeValue(dejaEvent, params.value);
               if (strText == null) {
                  this.logTraceToBackground(constants.LOG_WARNING,"dispatchEvents - strText is null for change event. Discarding queue entry.... [Type="+domEvent.type+"][Remaining_events_in_queue="+queue.length+"]");
                  //FIXME - Should we send a notification to the background?? Otherwise the replay will hang, right????
                  return;
               }
               ...
               if (this.changeOptionValue( target, dejaEvent ) === 0) {
                  this.logTraceToBackground(constants.LOG_WARNING,"Replay failure while processing dispatch event [Change]. Returning ST97. ");
            ...
            } else if (domEvent.type == "change" && dejaEvent.type == "changefile") {  // change file input
               //UXM-10587 - New feature for supporting upload files feature.
               this.pendingEvent = 'change';
               if ( dejaEvent.eventparams && dejaEvent.eventparams.filelistref ) {
                  var fileListRef = dejaEvent.eventparams.filelistref;
                  if ( this.fileLists && this.fileLists.has(fileListRef) ) {
                     var dataTransfer = this.fileLists.get(fileListRef);
                     target.files = dataTransfer.files; 
                  } else {
                     this.logTraceToBackground(constants.LOG_WARNING,"Replay failure while processing dispatch event [change file]. Returning script parse error. ");

                     this.logger.logFailure('dispatchEvents: file list not found ' + fileListRef);                     
                     this.handleReplayFailure("dcFailure_populateInputFile_fileListNotFound", null, constants.STATUS_SCRIPT_PARSE_ERROR); 
                     return;
                  }
               } else {
                  this.logTraceToBackground(constants.LOG_WARNING,"Replay failure while processing dispatch event [change file]. File missing. Returning script parse error. ");

                  this.logger.logFailure('dispatchEvents: file list reference missing at change file event ');                     
                  this.handleReplayFailure("dcFailure_populateInputFile_fileListRefNotFound", null, constants.STATUS_SCRIPT_PARSE_ERROR); 
                  return;
               }
               ...
               if ((domEvent.type == "keypress") && domEvent.charCode) {
                  ...
                  var fireEnter = domEvent.charCode === 13 && isShadowElement;
                  var refireEnter = typeof dejaEvent.eventparams.keycodes === 'undefined' && dejaEvent.attributes.simulateenter;
                  refireEnter = refireEnter && isShadowElement && target.value === dejaEvent.eventparams.value;
                  refireEnter = refireEnter && typeof dejaEvent.eventparams.shadowValue !== 'undefined';
                  if(fireEnter || refireEnter){
                     target.value = this.dispatchState.shadowValue;
            ...
            if ( this.logger.debugpending ) {
               this.logTraceToBackground(constants.LOG_DEBUG,"dispatchEvents - Event dispatched to target! [Type="+domEvent.type+"][Remaining_events_in_queue="+queue.length+"]");
            }
            ...
            this.logger.logWarning("dispatchEvents - Discarding event "+domEvent.type+". Target search to revalidate element failed. ");
            this.logTraceToBackground(constants.LOG_WARNING,"dispatchEvents - Browswer event discarded, element target not found during revalidation search. [Type="+domEvent.type+"][Remaining_events_in_queue="+queue.length+"]");
         }
         ...
         if (queue.length !== 0) {
            if ( this.logger.debugpending ) {
               this.logTraceToBackground(constants.LOG_DEBUG,"dispatchEvents - Still have elements to proceed. Sleeping for "+this.dispatchState.delay+"ms to proceed again. [Type="+domEvent.type+"][Remaining_events_in_queue="+queue.length+"]");
            }           
            ...
            this.varName = null;
            if ( this.logger.debugpending ) {
               this.logTraceToBackground(constants.LOG_DEBUG,"dispatchEvents - Queue processed. Sending dispatchComplete message to background. [Type="+domEvent.type+"]");
            }
         }

      } catch (ex) {
         this.logTraceToBackground(constants.LOG_FAILURE,"dispatchEvents - Unexpected error processing events queue: "+ex);
   ...
   scrollToNode: function (aNode, opt_offsetX, opt_offsetY) {
      ...
      if ((this.lastScrollTarget == null) ||
         ...
         while (obj.offsetParent !== null) {
            ...
            if ( ! obj ) {
               this.logger.logFailure("Parent is null at scrollToNode!");
               break;
            }
         }
   ...
   keywordSearch : function (aValidator)
   {
      ...
      try {
         ...
         if (this.activePage) {
            var oRegExText = aValidator.oRegExText;
            if ( aValidator.scriptVarInfo ) {
               var strMatchText;
               if ( aValidator.scriptVarInfo.sticky && aValidator.scriptVarInfo.varText ) {
                  strMatchText = aValidator.scriptVarInfo.varText;
               } else {
                  strMatchText = this._variable.computeScriptVariable(aValidator.scriptVarInfo.varName, aValidator.scriptVarInfo.varValue);
               }
               
               if ( strMatchText && strMatchText.length > 0 ) {
                  aValidator.oRegExText = strMatchText;

                  if (aValidator.bFixSpaces ) {
               ...
               oRegExText = strMatchText;
            }

            keywordFound = this.documentRegexKeywordSearch(oRegExText, aValidator);
            
         } else if (this.logger.debugprocess) {
            this.logger.logDebug("Keyword - Non active page. Ignoring!");
         }
         ...
         this.logger.logDebug("Unexpected error while processing keyword search: "+e);
      }
   ...
   /**
    * Generic function to try to find keyword in current document.
    * 
    * @param {*} oRegExText 
    * @param {*} aValidator 
    */
   documentRegexKeywordSearch : function(oRegExText, aValidator) {

      let keywordFound = false;

      if (!oRegExText || oRegExText.length < 1) {
         this.logger.logDebug("Keyword text/regex is undefined!");
      } else { 
         ...
         try { oRegEx = new RegExp(oRegExText, aValidator.strOptions); } catch (err) {}

         if (oRegEx.test(strHtmlText)) {
            keywordFound = true; // keyword found, exit doc search loop
            if (this.logger.debugprocess) {
               this.logger.logDebug("Keyword - Keyword found!");
            }
         } else if (this.logger.debugprocess) {
            this.logger.logDebug("Keyword - Keyword NOT found!");
         }
      }
   ...
   onRunMode : function(data) {
      try {
         ...
         //Some interactions at the content script could be different is this is a server extension.
         this.isServer = data.server;
         ...
         this.jsDialogDetection = data.jsDialogDetection;
         //UXM-10729 - Configuration option to define the limit of mutations to track while recording.
         if ( data.maxMutations ) {
            this.maxMutations = data.maxMutations;
            if ( this.maxMutations > 0 ) {
               this.logger.logWarning("Mutations Limit defined [Config option 'mutationslimit']: "+this.maxMutations);
            } else {
               this.logger.logDebug("Disabling the limit for mutations [Config option 'mutationslimit']");
            }
         } else {
            this.logger.logDebug("No limit defined for mutations [Config option 'mutationslimit']");
         }
         ...
         switch (this._runMode) {
            ...
            case constants.RUNMODE_REPLAY:
               ...
               //UXM-10587 - Prepare file lists for upload fields
               if ( data.filelists && data.filelists.length > 0 ) {
                  this.prepareUploadFileLists(
                        { 
                           listsdate: data.filelistsdate,
   ...
   /**
    * DOMNodeInsert event handler.
    * 
    * 
    * @param {*} evt 
    */
   onDOMInsertFromDOMListener : function( evt )
   {
      if ( evt ) {
         this.onDOMInsert(evt.target);
      } else {
         this.logger.logWarning("[onDOMInsertFromDOMListener] Received null event info!");
      }
   },
    ...
    * @param {*} mutations 
    */
   onDOMInsertFromMutationObserver : function( mutations )
   {
      if ( mutations ) {
         mutations.forEach(function(mutation) {
            for (var i = 0; i < mutation.addedNodes.length; i++){
               this.onDOMInsert(mutation.addedNodes[i]);
            }  
         }, this);
      } else {
         this.logger.logWarning("[onDOMInsertFromMutationObserver] Received null mutations info!");
      }
   ...
   onDOMInsert : function( evtTarget )
   {
      ...
      if (evtTarget && this._runMode == constants.RUNMODE_RECORD ) {
         if (evtTarget.id == this.DEJA_DIALOG_ID) {
            var value = JSON.parse(evtTarget.value);            
         ...
         else if (evtTarget.id == this.DEJA_OVERRIDE_FUNC_ID) {
            return;
         } 
         else if (evtTarget.id == this.DEJA_SINGLE_PAGE_APP_ID) {
             return;
	 }
      }
...
*/
      ...
      if (evtTarget.nodeType == 3) {  // if text target, bump up to parent
         var newTarget;
         try { newTarget = evtTarget.parentNode; } catch(ex) {}
         if (newTarget && newTarget.nodeType == 1) {  // only retarget if an ELEMENT type
            this.fixupDomNode( newTarget );
         }
      } else {
         this.fixupDomNode( evtTarget );
      }
...
*/
   ...
   INTEGRATED_MODE_URL: "goalert?source=uxm_integrated_deja",
...
//         this.fixupDomCount = 0;
         ...
         //UXM-10729 - Use MutationObserver if available.
         /* 
          * [UPDATE 04/02/2020] UXM-11294 - I have rolled back UXM-10729 changes 
          * for Chrome because it seems that a few amount of monitors are having some 
          * replay failures because of it.
          * I am almost sure that it could be fixed just with a MutationObserver configuration change
          * but, for the moment, I keep it just enabled for Firefox Quantum.
          */
         if ( this.isFirefox  && window.MutationObserver ) {
            this.mutationObservers = new DejaClick.EventRegistration().addMutationObserver(document, this.onDOMInsertFromMutationObserver, this);
            this.logger.logDebug("Added mutation observer [Target="+document+"]");
         } else {
            this.onDOMInsertListeners = new DejaClick.EventRegistration().
                  addDomListener(document, "DOMNodeInserted", true, this.onDOMInsertFromDOMListener, this);
            if ( this.isFirefox ) {
               this.logger.logWarning("Added DOMNodeInserted listener. MutationObserver not available [Target="+document+"]");
            }
         }
         ...
         //UXM-10893 - I've changed from "webkitVisibilityState" to "visibilityState" (In theory supported by Chrome: https://developer.mozilla.org/en-US/docs/Web/API/Document/visibilityState)
         var isVisible = (document.visibilityState == "visible") ? true  : false;
         if (!isVisible && !isFrame) {
            this.visibilityChangeListeners = new DejaClick.EventRegistration().
               //UXM-10893 - I've changed from "webkitvisibilitychange" to "visibilitychange" (In theory supported by Chrome: https://developer.mozilla.org/en-US/docs/Web/API/Document/visibilityState)
               addDomListener(document, "visibilitychange", true, this.handleVisibilityChange, this);
         }
   ...
   _onLoaded : function(aEvent) {
      var v_timings = {}, v_resources = [], v_paint = {};
      try {
         ...
         if (!this.isNewTabURL(document.URL) && window.self == window.top  && document.URL.indexOf('chrome-extension://') != 0) {
         ...
         window.performance.getEntriesByType('paint').forEach(function(entry) {
           v_paint[entry['name']] = entry['startTime'];
         }, this);

         this.observerService.notifyObservers('dejaclick:w3ctimings', {
               url: document.URL,
               timings: v_timings,
               paintTimings: v_paint['first-paint'] !== undefined ? v_paint : null,
   ...
   onUnloaded : function(aEvent) {
      try {
         ...
         if (window.self == window.top && !this.isNewTabURL(document.URL) && document.URL.indexOf('chrome-extension://') != 0) {
...
/**
 * Generate a generic input event
 * @return {event} an input event
 */
var inputEvent = ()=>{
...
/**
 * Generate a generic enter keydown event
 * @return {event} an enter keydown event
 */
var enterEvent = ()=>{
