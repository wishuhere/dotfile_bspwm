...
DejaClickUi.JsValidation = function (aOptions, aRootElement, aWindow,
    ...
    console.log('clicked! init js');
    aWindow.returnValue = null;
    this.jsCloseCallback = aOptions.callback ? aOptions.callback : null; 
...
//      addChromeListener(aWindowsApi.onRemoved, this.removeWindow, this);
    ...
    this.elements = {
        ...
        description: root.find('#jsDescription'),
        ...
        selectDocument: root.find('#jsSelectDocument'),

        jsApply: root.find('#jsApply'),
        jsRemove: root.find('#jsRemove'),
        jsCancel: root.find('#jsCancel'),

        allInputs: root.find('textarea,input,select'), // removed 'a' element
        changeInputs: root.find('#jsText,#evalDefault,#evalOne'),
        jsButtons: root.find('button'),
    ...
    root.find('#jsDisplayLevel').empty();
    this.displayLevel = new ADisplayLevelSelector(
        'x',
        root.find('#jsDisplayLevel')[0],
    ...
    if (this.item == null) {
        ...
        if($('body').is('#jsValidationDialog')){
            DejaClick.service.__modal.setTitle('deja_javascript_addTitle');
        ...
        this.elements.jsText.val((aOptions.jsText == null) ? '' :
            aOptions.jsText);
        this.elements.jsText.attr('rows',10); //UXM-10794 - Let's increase the default size of JavaScript val text field!
        ...
        this.elements.jsRemove.hide();

    } else {
        if($('body').is('#jsValidationDialog')){
            DejaClick.service.__modal.setTitle('deja_javascript_editTitle');
    ...
    this.elements.jsButtons.button();
    this.elements.jsApply.button();
    this.elements.selectDocument.on('click', this.selectDocument.bind(this));
    this.elements.jsApply.on('click', this.jsApply.bind(this));
    this.elements.jsRemove.on('click', this.jsRemove.bind(this));
    this.elements.jsCancel.on('click', this.jsCancel.bind(this));
    this.enableControls();
};
...
DejaClickUi.JsValidation.prototype = {
   ...
   jsClose: function () {
      if (this.hasOwnProperty('elements')) {
         ...
         this.elements.jsButtons.off('click').button('destroy');
      }
   ...
   jsApply: function (aEvent) {
      ...
      try {
         ...
         win = this.window;
         if(this.jsCloseCallback){
            this.jsCloseCallback();
          }
         if($('body').is('#jsValidationDialog')){
            this.jsClose();
            win.close();
   ...
   enableControls: function () {
      if (this.dialog == null) {
         ...
         this.elements.jsButtons.button('option', 'disabled', false);
         if (this.elements.evalDefault.prop('checked')) {
         ...
         if (this.variableSelector.isVariableUsed() ?
            ...
            this.elements.jsApply.button('option', 'disabled', true);
         }
         ...
         this.elements.jsButtons.button('option', 'disabled', true);
      }
   },
...
/**
    ...
    */
   function dejaJavascriptunload(aEvent) {
    try {
       if (DejaClickUi.hasOwnProperty('jsValidation')) {
          DejaClickUi.jsValidation.jsClose();
  ...
  */
 function dejaJavascriptInitialize(args) {
     // rootelement is validationcontainer if called from sidebar
     var rootElement = args ? $('#jsValidationContainer') : document.documentElement;
    ...
    try {
       ...
       $(window).on('unload', dejaJavascriptunload);
       if($('body').is('#jsValidationDialog')){
            DejaClick.service.__modal.resizeModal($('body').outerHeight() + 150);
       }       

       return DejaClickUi.jsValidation;
   ...
   try {
      if($('body').is('#jsValidationDialog')){
        if (DejaClick.service.__modal) {
            dejaJavascriptInitialize();
          } else {
             window.onDialogArguments = dejaJavascriptInitialize;
          }
      }
