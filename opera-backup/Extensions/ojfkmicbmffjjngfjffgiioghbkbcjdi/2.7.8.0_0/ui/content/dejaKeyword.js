...
DejaClickUi.KeywordValidation = function (aOptions , aRootElement,
   ...
   this.keywordCloseCallback = aOptions.callback ? aOptions.callback : null; 
   ...
   this.isTrigger = aOptions.isTrigger; //UXM-11948 - Reuse Keyword edit screen for subscript triggers too.
   ...
   this.elements = {
      ...
      description: root.find('#KeywordDescription'),
      ...
      selectDocument: root.find('#keywordSelectDocument'),

      keywordApply: root.find('#keywordApply'),
      keywordRemove: root.find('#keywordRemove'),
      keywordCancel: root.find('#keywordCancel'),

      allInputs: root.find('textarea,input,select,a'),
      changeInputs: root.find('#matchText,#plainText,#regExp,#searchAll,#searchOne'),
      keywordButtons: root.find('button'),
      advancedOnly: root.find('.advancedOnly'),
      validationsOnly: root.find('.validationsOnly')
   };
   aUtils.localizeTree(aRootElement, 'deja_');
   root.find('#variableDiv').empty();
   
   if ( this.isTrigger )
      root.find('#variableDiv').hide();
   ...
   root.find('#keywordDisplayLevel').empty();
   this.displayLevel = new ADisplayLevelSelector(
      'x',
      root.find('#keywordDisplayLevel')[0],
   ...
   if (this.item == null) {
      ...
      if($('body').is('#keywordValidationDialog')){
            DejaClick.service.__modal.setTitle(this.isTrigger?'deja_trigger_addTitle':'deja_keyword_addTitle');
      } else {
            this.elements.title.text(aUtils.getMessage(this.isTrigger?'deja_trigger_addTitle':'deja_keyword_addTitle'));
      }
      ...
      this.elements.keywordRemove.hide();

   } else {
      if($('body').is('#keywordValidationDialog')){
            DejaClick.service.__modal.setTitle(this.isTrigger?'deja_trigger_editTitle':'deja_keyword_editTitle');
      } else {
            this.elements.title.text(aUtils.getMessage(this.isTrigger?'deja_trigger_editTitle':'deja_keyword_editTitle'));
      ...
      this.elements.keywordRemove.show();
   }

   console.log('clicked! init keywords');
   ...
   this.elements.keywordButtons.button();
   this.elements.keywordApply.button();
   this.elements.selectDocument.button();
   this.elements.selectDocument.on('click', this.selectDocument.bind(this));
   $('#keywordApply').off('click');
   $('#keywordApply').on('click', this.keywordApply.bind(this));
   this.elements.keywordRemove.on('click', this.keywordRemove.bind(this));
   this.elements.keywordCancel.on('click', this.keywordCancel.bind(this));
   this.enableControls();
};
...
DejaClickUi.KeywordValidation.prototype = {
   ...
   keywordClose: function () {
      if (this.hasOwnProperty('elements')) {
         ...
         this.elements.keywordButtons.off('click').button('destroy');
      }
    ...
    */
   keywordApply : function (aEvent) {
     
      var parent, targets, win;
      try {
         if (this.item == null) {
            parent = this.script.getChildWithTag(this.context, this.isTrigger?'triggers':'validations');
            if (parent == null) {
               parent = this.script.domTreeInsertNode(this.context,
                  this.isTrigger?'triggers':'validations');
            }
         ...
         win = this.window;
          ...
          if(this.keywordCloseCallback){
            this.keywordCloseCallback();
          }
          if($('body').is('#keywordValidationDialog')){
            this.keywordClose();    
            win.close();
   ...
   displayDisplayLevel: function (aLevel) {
      try {
         ...
         if ( this.isTrigger ) {
            this.elements.validationsOnly.hide();
         }
      } catch (ex) {
         this.logger.logException(ex);
      }
   ...
   enableControls: function () {
      if (this.dialog == null) {
         ...
         this.elements.keywordButtons.button('option', 'disabled', false);
         ...
         if (this.variableSelector.isVariableUsed() ?
            ...
            this.elements.keywordApply.button('option', 'disabled', true);
         }
         ...
         this.elements.keywordButtons.button('option', 'disabled', true);
      }
   },
...
/**
    ...
    */
   function dejaKeywordunload(aEvent) {
      try {
         if (DejaClickUi.hasOwnProperty('keywordValidation')) {
            DejaClickUi.keywordValidation.keywordClose();
    ...
    */
   function dejaKeywordInitialize(args) {

         // rootelement is validationcontainer if called from sidebar
         var rootElement = args ? $('#keywordValidationContainer') : document.documentElement;

         var args = args ? args : DejaClick.service.__modal.arguments;
      try {
         if(DejaClickUi.keywordValidation){
            delete DejaClickUi.keywordValidation;
         }
         ...
         return DejaClickUi.keywordValidation;
      } catch (ex) {
            console.log(ex);
         DejaClick.utils.logger.logException(ex);
      }
      ...
      try {
            if($('body').is('#keywordValidationDialog')){
                  if (DejaClick.service.__modal.arguments) {
                        dejaKeywordInitialize();
                  } else {
                     window.onDialogArguments = dejaKeywordInitialize;
                  }
            }
