...
const ChannelManager = {
  ...
  makeRoutingId(sender) {
    const tabId = sender?.tab.id ?? -1;
    const frameId = sender?.frameId ?? 0;
    return [tabId, frameId].join(':');
  },
...
const ServiceRegistry = {
  ...
  app() {
    const routingId = ['app', ++this.appCount].join(':');
    return {
      metadata: { routingId },
      handler: handleAppMessage,
    };
  },
  dispatcher(port) {
    const metadata = {
      ...
      routingId: ChannelManager.makeRoutingId(port.sender),
    };
    return {
      metadata,
      handler: handlePageMessage,
    };
  },
};

function bindInterface(port) {
  const binder = ServiceRegistry[port.name];
  if (!binder) return;
  const { metadata, handler } = binder.call(ServiceRegistry, port);
...
chrome.runtime.onConnect.addListener(bindInterface);
